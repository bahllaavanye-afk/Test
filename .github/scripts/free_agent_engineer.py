...
def _create_fix_branch(issue_number: int, issue_title: str) -> str:
    """Checkout a new branch for the fix. Returns the branch name."""
    ...
    print(f"[free-agent] Created fix branch {branch} from {base}")
    return branch
...
def _create_pull_request(
    ...
    """Open a PR from branch → claude/advanced-trading-bot-d5Lmw. Returns PR URL."""
    if not GH_TOKEN or not GH_REPO:
        return None
    try:
        base_branch = "claude/advanced-trading-bot-d5Lmw"
        pr_body = (
            f"Closes #{issue_number}\n\n"
            f"**Root cause:** {fix.get('root_cause', 'N/A')}\n\n"
            f"**What was changed:** {fix.get('explanation', 'N/A')}\n\n"
            f"**File patched:** `{fix.get('file_path', 'N/A')}`\n\n"
            f"_Auto-generated by {agent_name} (free-agent-engineer)_"
        )
        payload = {
            "title": f"fix(auto): {issue_title}",
            "head": branch,
            "base": base_branch,
            "body": pr_body,
        }
        ...
        pr_url = resp.get("html_url") or resp.get("url", "")
        print(f"[free-agent] Opened PR: {pr_url}")
        return pr_url
    except Exception as e:
        print(f"[free-agent] PR creation failed: {e}")
        return None
...
def _process_issue(issue: dict) -> str:
    ...
    # 6. Create a dedicated fix branch (before committing)
    try:
        fix_branch = _create_fix_branch(number, title)
    except subprocess.CalledProcessError as e:
        print(f"[free-agent] Branch creation failed: {e.stderr}")
        _revert_file(fix["file_path"])
        return "failed:git_branch"

    # 7. Commit on the fix branch
    if not _commit_fix(fix["file_path"], title, agent_name):
        _revert_file(fix["file_path"])
        _git("checkout", "claude/advanced-trading-bot-d5Lmw", check=False)
    ...
    # 9. Open a PR
    pr_url = _create_pull_request(fix_branch, number, title, fix, agent_name)

    # 10. Comment on the issue with a link to the PR (leave open until PR merges)
    if pr_url:
        pr_comment = (
            f":robot: **Auto-fix PR opened by {agent_name}**\n\n"
            f"**Root cause:** {fix.get('root_cause', 'N/A')}\n\n"
            f"**What was changed:** {fix.get('explanation', 'N/A')}\n\n"
            f"**File patched:** `{fix.get('file_path', 'N/A')}`\n\n"
            f"PR: {pr_url}\n\n"
            f"_This issue will be closed automatically when the PR is merged._"
        )
        _comment_on_issue(number, pr_comment)
    else:
        # PR creation failed — fall back to closing with commit info
        close_comment = (
            f":white_check_mark: **Auto-fixed by {agent_name}**\n\n"
            f"**Root cause:** {fix.get('root_cause', 'N/A')}\n\n"
            f"**What was changed:** {fix.get('explanation', 'N/A')}\n\n"
            f"**File patched:** `{fix.get('file_path', 'N/A')}`\n\n"
            f"Commit pushed to `{fix_branch}`."
        )
        _close_issue(number, close_comment)

    # 11. Notify Slack #incidents
    pr_ref = f" | PR: {pr_url}" if pr_url else ""
    slack_msg = (
        f":robot: *Free-agent engineer opened fix PR for issue #{number}*\n"
        f"*Issue:* {title}\n"
        f"*Agent:* {agent_name}\n"
        f"*File:* `{fix['file_path']}`\n"
        f"*Root cause:* {fix.get('root_cause', 'N/A')}\n"
        f"*Fix:* {fix.get('explanation', 'N/A')}{pr_ref}"
    )
