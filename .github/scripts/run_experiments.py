...
def _compute_metrics(entries: pd.Series, exits: pd.Series, prices: pd.Series) -> dict:
    """Compute strategy metrics from boolean entry/exit signals and close prices."""
    ...
    ann_factor = 252 if len(prices) > 252 else len(prices)
    mean_r = strat_returns.mean() * ann_factor
    std_r  = strat_returns.std() * (ann_factor ** 0.5)
    sharpe = float(mean_r / std_r) if std_r > 0 else 0.0

    neg_r  = strat_returns[strat_returns < 0]
    sortino_denom = neg_r.std() * (ann_factor ** 0.5) if len(neg_r) > 0 else 1e-9
...
def _run_one(name: str, module_path: str, class_name: str, symbol: str,
    ...
    metrics = _compute_metrics(entries[test_mask], exits[test_mask], df["close"][test_mask])
    print(
...
def main() -> None:
    ...
    successes.sort(key=lambda r: r["results"]["sharpe"], reverse=True)
    lines.append("*Top results by Sharpe:*")
    for r in successes[:10]:
        exp   = r["experiment"]
        res   = r["results"]
        emoji = "🟢" if res["sharpe"] > 1.0 else ("🟡" if res["sharpe"] > 0.5 else "🔴")
        lines.append(
            f"{emoji} `{exp['strategy']}/{exp['symbol']}` "
            f"Sharpe={res['sharpe']:+.3f}  MDD={res['max_drawdown']:+.1%}  "
