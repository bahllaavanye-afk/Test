...
- [ ] **[P0] Render deploys fail at STARTUP (update_failed / earlyExit)** — IN PROGRESS 2026-07-19. Confirmed: build succeeds, app exits `nonZeroExit:1` then Render logs `server_failed earlyExit:true`. **SECRET_KEY is NOT the cause** (deploy-on-main's new ensure-step reported it "already valid on Render, len 44" — my earlier hypothesis was wrong; the SECRET_KEY safety step is harmless/idempotent and stays). The app boots cleanly LOCALLY with a valid SECRET_KEY on SQLite AND survives an unreachable Postgres, so the crash is some prod-only condition the events API doesn't expose. NEXT: deploy-on-main.yml now (a) fixed a bash syntax bug in the same-commit dedupe that was aborting the deploy step with exit 2, and (b) fetches the **actual Render application logs** (the Python traceback) on failure — read the next failed run's CI log for the real boot error, then fix. Suspects still open: Redis/Upstash client at startup, a prod dep, or a lifespan task that raises only against the real Supabase/Upstash.
