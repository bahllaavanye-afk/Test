...
- [ ] **[P2] Discord reactions are a silent no-op** — `_react()` (👀 on receipt, ✅ on success, ❌ on failure) calls `chat_call("reactions.add", …)`, which has no Discord implementation because it needs a real message id and the Slack-era `ts` is not one. Agents therefore never acknowledge summons visually. Wire it to `PUT /channels/{id}/messages/{msg_id}/reactions/{emoji}`, which first requires `chat_call` to return the real message id it currently drops.
