...
- [x] **[P2] Inactive assets are still requested** (`MKR/USD is not active`, 422). ~~The universe should be filtered against Alpaca's active-asset list before signals are generated, not discovered at order time.~~ **The proposed fix was impossible and the premise was wrong.** Proven live 2026-07-28 (runs `30401244361`, `30402044105`): `/v2/assets?asset_class=crypto&status=active` returns MKR/USD **as tradable** while `POST /v2/orders` refuses it as not active — Alpaca's metadata contradicts its own order engine, so filtering against that list can never catch this. Fixed instead by remembering the refusal: a run-scoped memory (no repeat submissions within a run) plus a checked-in, 7-day-expiring denylist (`.github/state/inactive_assets.json`) applied **before** signal generation, so the symbol stops consuming a scarce top-K slot. See CONTINUITY.md 2026-07-28 22:00.
