...
- [x] **[P0] Render deploys fail at STARTUP (update_failed)** — ROOT-CAUSED + FIXED 2026-07-19. Real error (from the Render event dump + a local reproduction): build succeeds, then the app exits `nonZeroExit:1` because **SECRET_KEY is not set on the Render service** → `app/config.py` hard-raises at import (`SECRET_KEY must be a secure 32-byte hex` when TRADING_MODE=paper) → uvicorn exits 1 before binding. render.yaml has `generateValue:true` but `autoDeploy:false` means that blueprint env was never synced. FIX (deploy-on-main.yml): a new step sets a secure SECRET_KEY on Render if absent/placeholder/short (idempotent; uses the `SECRET_KEY` GitHub secret if provided, else generates one), AND the same-commit dedupe now only skips when the commit is already **live** (a prior FAILED deploy of the same SHA no longer blocks the recovery redeploy). Verify: the next deploy-on-main run should reach `live` and /health should show the fresh build. THEN the post-deploy revival chain (bots seeded, scheduler advancing, Trades rows) can proceed.
...
- [ ] **[P0] Desk fills → backend Trades attribution** (this is why the website shows "no trades") — CONFIRMED 2026-07-19: desks DO trade (latest Sunday run placed+filled a real DOGE/USD crypto order; equities/options correctly logged "market closed" on the weekend), but the fills live in Alpaca and never become backend `Trade` rows, so the site/leaderboard look empty. order_sync/fill attribution must ingest desk orders (client_order_id prefix qe-*) so dashboards reflect REAL desk activity. Until then: crypto trades 24×7, equities only on weekdays 9:30–16:00 ET — so low weekend volume is expected, not a bug.
