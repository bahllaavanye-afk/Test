...
- [ ] **[P1] Free-LLM cascade is one provider deep.** Only `GEMINI_API_KEY` is populated; `GROQ`, `DEEPSEEK`, `SAMBANOVA`, `CEREBRAS`, `TOGETHER`, `HYPERBOLIC`, `NVIDIA_NIM` are all empty in the live workflow env. A Gemini rate-limit therefore means no LLM at all. Also `.github/state/llm_metrics.jsonl` does not exist — the recorder in `llm_common._record_metric` has never written a line, so there is zero observability on provider success.
