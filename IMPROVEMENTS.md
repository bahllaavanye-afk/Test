...
>
> Remaining: `execution/` polling loops (vwap/iceberg/rl_exec) still warn-and-continue on quote failures — lower stakes than the bracket layer but the same "protection quietly stops working" shape; and none of these error paths page Discord yet — worth a judgement call per case. Note structlog does NOT propagate to stdlib logging, so assertions about these logs must use capsys, not caplog (caplog passes for the wrong reason).
