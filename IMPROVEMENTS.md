...
- [ ] **[P2] Inactive assets are still requested** (`MKR/USD is not active`, 422). The universe should be filtered against Alpaca's active-asset list before signals are generated, not discovered at order time. **Status 2026-07-28 23:45: the filter existed but had never run** — `_filter_tradable_crypto` was only called from `run_desk()`, which has zero call sites. Now wired into the real pipeline, along with a run-scoped memory of broker refusals and a 7-day-expiring denylist (`.github/state/inactive_assets.json`) applied before signal generation so a dead symbol stops consuming a scarce top-K slot. **Still open** because it is not yet known whether Alpaca lists MKR/USD as active — I claimed it did, from a missing log line that a dead code path could never have printed. The next live run answers it. See CONTINUITY.md 2026-07-28 23:45.
- [ ] **[P1] The strategy trimmer never affected live trading.** `_trimmed_strategies()` was also reachable only from the dead `run_desk()`, so strategies retired for poor performance kept trading and the whole pruning loop was decorative. Wired into the signal-generation stage 2026-07-28 23:45; **verify on a live run** that retired strategies are named in the `✂ ... will not trade` line and produce no orders.
...
- [ ] **[P2] Original note: test-isolation flake pattern** — two known cross-file flakes under `-n 4 --dist loadfile` (breakeven_inflation contract, tearsheet-on-sqlite): shared per-worker DB lets one file's rows leak into another's expectations. Fix: per-file DB fixtures or scoped assertions (lifecycle tests already scope per-bot).
