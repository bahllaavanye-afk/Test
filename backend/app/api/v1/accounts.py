...
async def latest_total_equity(db: AsyncSession) -> float:
    ...
    # Subquery to fetch the latest timestamp per account
    latest_ts_subq = (
        select(
            AccountSnapshot.account_id,
            func.max(AccountSnapshot.ts).label("max_ts"),
        )
        .group_by(AccountSnapshot.account_id)
        .subquery()
    )

    # Join the latest timestamps with their corresponding equity values,
    # limited to active accounts.
    equity_sum_stmt = (
        select(func.coalesce(func.sum(AccountSnapshot.total_equity), 0.0))
        .join(
            latest_ts_subq,
            (AccountSnapshot.account_id == latest_ts_subq.c.account_id)
            & (AccountSnapshot.ts == latest_ts_subq.c.max_ts),
        )
        .join(Account, Account.id == AccountSnapshot.account_id)
        .where(Account.is_active.is_(True))
    )

    result = await db.execute(equity_sum_stmt)
