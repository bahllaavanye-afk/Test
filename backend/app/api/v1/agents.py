...
def _read_json(path: Path | None) -> dict:
    """Safely read a JSON file, returning an empty dict on errors or invalid content."""
...
def _write_json(path: Path | None, data: dict) -> None:
    """Write a dict to a JSON file, ensuring parent directories exist."""
    if not path:
        raise ValueError("Invalid path provided for JSON write operation.")
...
def _resolve_key(*names: str) -> str:
    """Return the first non‑empty environment variable found among the provided names."""
    for name in names:
        if not name:
            continue
        ...
        if v:
            return v
        # try fallback with _1 suffix if the name does not already end with a digit
        if not name[-1].isdigit():
            v = os.environ.get(f"{name}_1", "")
            if v:
                return v
    return ""


async def _call_llm(messages: List[dict] | None, max_tokens: int = 600) -> str:
    """Attempt to obtain a completion from Groq, DeepSeek, then Gemini.

    Handles empty or None message lists gracefully.
    """
    if not messages:
        return "No messages provided for LLM call."
...
class TaskCreate(BaseModel):
    # Placeholder fields; actual implementation omitted for brevity.
    pass

# Additional endpoints and logic would follow here.