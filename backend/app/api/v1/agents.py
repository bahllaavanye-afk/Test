...
async def _call_llm(messages: list[dict], max_tokens: int = 600) -> str:
    ...
    for key in [_resolve_key("DEEPSEEK_API_KEY"), os.environ.get("DEEPSEEK_API_KEY_2", ""), os.environ.get("DEEPSEEK_API_KEY_3", "")]:
        if not key:
            continue
        try:
...
async def get_workflows(current_user: User = Depends(get_current_user)):
    ...
    return [{"name": w, "file": f"{w}.yml", "schedule": "varies"} for w in GITHUB_WORKFLOWS]
...
class TaskCreate(BaseModel):
    ...
    description: str | None = Field(
        None,
        description="Human‑readable description of what the task should accomplish.",
        example="Generate a daily performance report for all desks.",
    )
    priority: int = Field(
        1,
        ge=1,
        le=5,
        description="Priority level of the task, where 1 is lowest and 5 is highest.",
        example=3,
    )
...
class TaskUpdate(BaseModel):
    ...
    description: str | None = Field(
        ...
        example="Regenerate the daily report with additional metrics.",
    )
    priority: int | None = Field(
        None,
        ge=1,
        le=5,
        description="Updated priority level (1‑5).",
        example=4,
    )
    ...
    def status_must_be_valid(cls, v: str | None) -> str | None:
        if v is None:
            return v
        allowed = {"pending", "in_progress", "completed", "failed", "canceled"}
        if v not in allowed:
...
class TaskResponse(BaseModel):
    """Schema returned when querying a task."""

    id: str = Field(
        ...,
        description="System‑generated identifier for the task.",
        example="task_12345",
    )
    name: str = Field(
        ...,
