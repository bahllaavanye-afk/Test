...
# Constants
ROUTER_PREFIX: str = "/backtests"
ROUTER_TAGS: list[str] = ["backtests"]
...
ERR_UNKNOWN_STRATEGY: str = "Unknown strategy: {}"
ERR_INSUFFICIENT_DATA: str = "Insufficient data for {} ({})"
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            if strategy_cls is None:
                ...
                run.error_message = ERR_UNKNOWN_STRATEGY.format(strategy_name)
            ...
            if df is None or df.empty or len(df) < MIN_DATA_POINTS:
                run.status = STATUS_FAILED
                run.error_message = ERR_INSUFFICIENT_DATA.format(symbol, interval)
                run.completed_at = datetime.now(timezone.utc)
                await db.commit()
                return
            ...
            result = BacktestResult(
                ...
                sharpe_ratio=round(metrics.sharpe, METRIC_PRECISION),
                sortino_ratio=round(metrics.sortino, METRIC_PRECISION),
                calmar_ratio=round(metrics.calmar, METRIC_PRECISION),
                max_drawdown=round(metrics.max_drawdown, METRIC_PRECISION),
                win_rate=round(metrics.win_rate, METRIC_PRECISION),
                profit_factor=round(metrics.profit_factor, METRIC_PRECISION),
                total_trades=metrics.num_trades,
                equity_curve=metrics.equity_curve[:EQUITY_CURVE_MAX_POINTS],
            )
            db.add(result)
