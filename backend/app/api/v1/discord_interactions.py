...
def _extract_options(data: dict) -> dict[str, str]:
    """Return a mapping of option name → value from a Discord command payload."""
    return {opt.get("name"): opt.get("value") for opt in (data.get("options") or []) if opt}


async def _handle_application_command(data: dict) -> dict:
    """Process a Discord application command and return a Discord message payload."""
...
async def discord_interactions(request: Request):
    ...
    if itype == _APPLICATION_COMMAND:
        return await _handle_application_command(payload.get("data") or {})
