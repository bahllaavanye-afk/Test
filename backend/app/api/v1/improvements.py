...
@router.get("/history")
async def get_history(_current_user: User = Depends(get_current_user)):
...
@router.get("/quality")
async def get_quality(_current_user: User = Depends(get_current_user)):
...
async def get_best_params(_current_user: User = Depends(get_current_user)):
    improver = getattr(app.state, "self_improver", None)
    if improver is None:
