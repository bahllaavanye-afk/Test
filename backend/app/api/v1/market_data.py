...
def _alpaca_headers() -> dict:
    """Generate Alpaca authentication headers."""
    return {
...
def _interval_to_alpaca(interval: str) -> str:
    """Map generic interval strings to Alpaca's naming convention."""
    mapping = {
        "1m": "1Min",
        "5m": "5Min",
        "15m": "15Min",
...
def _period_to_start(period: str) -> str:
    """Convert a period like '1mo' into an ISO‑8601 start timestamp."""
    days_map = {
        "1mo": 30,
        "3mo": 90,
        "6mo": 180,
...
async def get_quotes_batch(
    ...
    try:
        ...
        logger.debug("batch quote fetch failed", symbols=",".join(sym_list), error=str(exc))

    # If the batch endpoint fails, fall back to individual fetches to maximise data availability.
    fallback_results = []
    for sym in sym_list:
        quote = await get_quote(sym, current_user)
        fallback_results.append(quote)
