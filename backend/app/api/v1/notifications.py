...
def _verify_slack_signature(
    ...
    Restores a Codex P1 security finding that regressed: without this,
    anyone can POST forged events to /slack/events.
...
async def get_activity(
    ...
    """Return recent activity tracked by the system."""
...
async def slack_test(current_user: User = Depends(get_current_user)):
    ...
    ok = await slack.notify_system(
        "QuantEdge Slack notifications are working ✓",
        level="info",
    )
        ...
        async with httpx.AsyncClient(timeout=8.0) as client:
            resp = await client.get(
                ...
                params={
                    "channel": channel_id,
                    "ts": thread_ts,
                    "limit": limit + 1,  # include the parent message
                },
            )
            data = resp.json()
            if not data.get("ok"):
            ...
            # Exclude the parent message (the first one) to keep only recent replies
            recent = msgs[1 : limit + 1] if len(msgs) > 1 else []
            texts = [msg.get("text", "").strip() for msg in recent if msg.get("text")]
            return "\n".join(texts)
    except Exception as _e:
        logger.debug(
