...
# ── Constants ─────────────────────────────────────────────────────────────────
REPLAY_TOLERANCE_SECONDS = 60 * 5  # 5 minutes
DEFAULT_ACTIVITY_LIMIT = 100
MAX_ACTIVITY_LIMIT = 500

SLACK_TEST_MESSAGE = "QuantEdge Slack notifications are working ✓"
...
DEFAULT_QA_REPORT_PATH = "qa_health_report.json"
DEFAULT_SLACK_THREAD_LIMIT = 4

SLACK_NOTIFICATION_TITLE = "📋 Employee Status Report"
SLACK_NOTIFICATION_CHANNEL = "system"
SLACK_NOTIFICATION_USER = "system"
...
def _verify_slack_signature(raw_body: bytes, timestamp: str, signature: str, secret: str) -> bool:
    ...
    if not (secret and signature and timestamp):
        return False
    try:
        if abs(time.time() - int(timestamp)) > REPLAY_TOLERANCE_SECONDS:  # reject stale (replay) requests
            return False
    except ValueError:
        return False
...
async def slack_test(current_user: User = Depends(get_current_user)):
    ...
    ok = await slack.notify_system(SLACK_TEST_MESSAGE, level="info")
...
async def post_employee_report(current_user: User = Depends(get_current_user)):
    ...
    if modeling:
        ...
        drifted = sum(
            1 for v in eng.get("latest_performance", {}).values()
    ...
    try:
        ...
        report_path = Path(os.getenv("QA_HEALTH_REPORT_PATH", DEFAULT_QA_REPORT_PATH))
...
async def _fetch_slack_thread_context(token: str, channel_id: str, thread_ts: str, limit: int = DEFAULT_SLACK_THREAD_LIMIT) -> str:
    ...
    try:
        ...
        logger.debug("slack thread fetch failed", channel_id=channel_id, thread_ts=thread_ts, error=str(_e))
    return ""