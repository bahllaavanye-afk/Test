...
async def _fetch_snapshots(symbols: list[str] | None) -> dict[str, dict]:
...
async def get_options_chain(
    ...
    if not symbol:
        raise HTTPException(400, "Symbol parameter cannot be empty")
...
async def get_options_snapshot(
    ...
    # Return a minimal structure when snapshot is missing
    if not snapshot:
        return {
            "symbol": symbol.upper(),
    ...
    # Enrich with contract‑level fields where possible (contract info may be missing)
    enriched = _enrich_contract({"symbol": symbol.upper()}, snapshot)
    return enriched