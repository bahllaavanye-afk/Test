...
async def _fetch_snapshots(symbols: list[str]) -> dict[str, dict]:
    ...
    async with httpx.AsyncClient(timeout=20.0) as client:
        ...
        tasks = [_fetch_batch(to_fetch[i : i + BATCH]) for i in range(0, len(to_fetch), BATCH)]
...
async def get_options_snapshot(
    ...
    if not settings.alpaca_api_key or not settings.alpaca_secret_key:
        raise HTTPException(400, "Alpaca API credentials not configured")

    symbol_up = symbol.upper()
    cached = _cache_get(symbol_up)
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        try:
            resp = await client.get(
                f"{_ALPACA_BASE}/v2/options/snapshots",
                params={"symbols": symbol_up, "feed": "indicative"},
                headers=_alpaca_headers(),
            )
        except httpx.RequestError as exc:
            raise HTTPException(502, f"Alpaca connection error: {exc}") from exc
    ...
    data = resp.json()
    snapshot = (data.get("snapshots") or {}).get(symbol_up)
    if snapshot:
        _cache_set(symbol_up, snapshot)
        return snapshot
