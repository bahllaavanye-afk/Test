...
def _normalize(payload: dict[str, Any]) -> dict[str, Any]:
    """Normalize TradingView's free‑form alert JSON into a consistent schema.

    Args:
        payload: Raw JSON payload received from TradingView.

    Returns:
        A dictionary with standardized keys: ``symbol``, ``side``, ``price``,
        ``strategy``, ``message``, and ``received_at``.
    """
    return {
        "symbol": str(payload.get("ticker") or payload.get("symbol") or "").upper()
        or None,
        "side": (
            str(payload.get("action") or payload.get("side") or "").lower() or None
        ),
        "price": _float_or_none(payload.get("price") or payload.get("close")),
        "strategy": payload.get("strategy") or payload.get("indicator"),
        "message": str(
            payload.get("message") or payload.get("comment") or ""
        )[:500] or None,
...
async def receive_tradingview_alert(request: Request) -> dict[str, Any]:
    """Receive a TradingView webhook alert, validate it, and fan‑out to Redis.

    The endpoint is disabled unless ``TRADINGVIEW_WEBHOOK_SECRET`` is set.
    """
    ...
    try:
        ...
        logger.debug(
            "tradingview alert: redis publish skipped", error=str(exc)
        )
