...
# Constants
ENV_SECRET_NAME = "TRADINGVIEW_WEBHOOK_SECRET"
DISABLED_DETAIL = "TradingView webhook receiver disabled — set TRADINGVIEW_WEBHOOK_SECRET."
INVALID_BODY_DETAIL = "Body must be a JSON object."
INVALID_SECRET_DETAIL = "Bad or missing webhook secret."
REDIS_CHANNEL = "tradingview:alerts"
...
def _normalize(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "message": str(payload.get("message") or payload.get("comment") or "")[:MAX_MESSAGE_LENGTH] or None,
...
async def receive_tradingview_alert(request: Request) -> dict:
    secret = os.environ.get(ENV_SECRET_NAME, "").strip()
    if not secret:
        raise HTTPException(
    ...
    try:
        ...
        if r is not None:
            ...
            await r.publish(REDIS_CHANNEL, _json.dumps(alert))
