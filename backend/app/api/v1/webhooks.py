...
def _get_secret() -> str:
    """Retrieve the webhook secret or raise if the endpoint is disabled."""
...
async def _parse_payload(request: Request) -> dict:
    """Parse JSON body from the request, ensuring it is a dict."""
    try:
...
def _verify_secret(payload: dict, secret: str) -> None:
...
def _record_alert(alert: dict) -> None:
    """Store alert in the ring buffer and update global counters."""
...
def _log_alert(alert: dict, exec_time: float) -> None:
    """Emit structured log entry for the received alert."""
    logger.info(
