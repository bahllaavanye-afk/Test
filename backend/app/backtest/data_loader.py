...
def _build_alpaca_params(
    ...
    """Construct query parameters for the Alpaca crypto bars endpoint."""
    params = {
        "symbols": pair,
        "timeframe": timeframe,
...
def _parse_alpaca_payload(payload: dict, pair: str) -> Tuple[List[dict], str | None]:
    """Extract rows and next page token from Alpaca response."""
    rows = (payload.get("bars") or {}).get(pair, [])
...
def _fetch_alpaca_crypto(
    ...
    # Crypto bars are public; sending keys (when present) raises rate limits.
    ...
    for _ in range(max_pages):
        ...
        rows, page_token = _parse_alpaca_payload(payload, pair)
