...
def _fetch_alpaca_crypto(
    ...
    # Crypto bars are public; using API keys can trigger stricter rate limits.
...
def _synthetic_ohlcv(symbol: str, start: date, end: date, interval: str) -> pd.DataFrame:
    ...
    # For non‑daily intervals we still return daily bars; Alpaca/YFinance will
    # resample as needed downstream.
...
def fetch_ohlcv(
    ...
    """Public entry point used by strategies/backtests.

    - Returns an empty DataFrame for invalid ranges.
    - Uses Alpaca for crypto, falling back to yfinance, then synthetic.
    - Caches results to avoid repeated network calls.
    """
    if start > end:
        logger.warning("fetch_ohlcv called with start > end; returning empty DataFrame")
        return pd.DataFrame()
    ...
    if market_type == "crypto":
        try:
            ...
            logger.debug(f"Alpaca fetch failed for {symbol}: {exc}")

    # Generic yfinance path.
    try:
        import yfinance as yf

        yf_symbol = _symbol_to_yf(symbol, market_type)
        yf_interval = _interval_to_yf(interval)
        # yfinance expects inclusive end dates; we add a day to ensure coverage.
        ...
        if not data.empty:
            # yfinance may return columns with different naming conventions.
    ...
    logger.info(f"Falling back to synthetic OHLCV for {symbol}")
