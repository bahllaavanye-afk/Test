...
def _request_alpaca(url: str, params: dict, headers: dict) -> dict:
    """Perform a single Alpaca request and return the parsed JSON payload."""
    import urllib.parse

    full_url = f"{url}?{urllib.parse.urlencode(params)}"
    return _http_get_json(full_url, headers, timeout=20.0)


def _extract_alpaca_bars(payload: dict, pair: str) -> tuple[list[dict], str | None]:
    """Return the list of bar dicts and the next page token from an Alpaca payload."""
    bars = (payload.get("bars") or {}).get(pair, [])
...
def _fetch_alpaca_crypto(
    ...
    # Build request headers; avoid sending keys unless they are explicitly set
    # (using test keys would trigger rate‑limits).
    ...
    for _ in range(max_pages):
        params = _build_alpaca_params(pair, start, end, interval, page_token)
        payload = _request_alpaca(_ALPACA_CRYPTO_BARS_URL, params, headers)
        bars, page_token = _extract_alpaca_bars(payload, pair)
        rows.extend(bars)
...
def _synthetic_ohlcv(symbol: str, start: date, end: date, interval: str) -> pd.DataFrame:
    ...
    # Add modest noise to open price to avoid perfect correlation
