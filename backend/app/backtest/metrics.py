"""
Optimized backperformance metrics.
...
VAR_PERCENTILE: int = 5  # 5th percentile corresponds to 95% VaR
...
def compute_metrics(
    ...
    total_return_pct = round(total_return * 100, ROUND_PRECISION)

    annual_return = (final / initial) ** (1.0 / max(years, EPSILON)) - 1.0
    annual_return_pct = round(annual_return * 100, ROUND_PRECISION)

    # ------------------------------------------------------------------
    # Sharpe (rf = 0)
    # ------------------------------------------------------------------
    sharpe = (
        float(daily_mean / daily_std * np.sqrt(TRADING_DAYS_PER_YEAR)) if daily_std > 0 else 0.0
    )
    ...
    sortino = (
        float(daily_mean / downside_std * np.sqrt(TRADING_DAYS_PER_YEAR))
        if downside_std > 0
        else 0.0
    )
    ...
    max_drawdown_pct = round(max_drawdown * 100, ROUND_PRECISION)

    if (drawdown_series < 0).any():
        avg_dd = float(drawdown_series[drawdown_series < 0].mean())
        avg_drawdown_pct = round(avg_dd * 100, ROUND_PRECISION)
    else:
    ...
    # ------------------------------------------------------------------
    if max_drawdown != 0:
        calmar = round(annual_return / abs(max_drawdown), ROUND_PRECISION)
        recovery_factor = round(total_return / abs(max_drawdown), ROUND_PRECISION)
    else:
        calmar = 0.0
        recovery_factor = 0.0
    ...
    if hasattr(equity.index, "to_period"):
        monthly = equity.resample(MONTHLY_RESAMPLE_FREQ).last()
        monthly_returns = monthly.pct_change().dropna()
    else:
        monthly_returns = pd.Series(dtype=float)

    if len(monthly_returns) > 0:
        best_month_pct = round(float(monthly_returns.max()) * 100, ROUND_PRECISION)
        worst_month_pct = round(float(monthly_returns.min()) * 100, ROUND_PRECISION)
    else:
        best_month_pct = 0.0
        worst_month_pct = 0.0
    ...
    # The remaining code continues to compute trade‑level metrics and
    # constructs the BacktestMetrics dataclass using the values calculated
    # above.
    # Ensure that any further magic numbers are also defined as constants
    # if new ones are introduced.

    # Placeholder return to satisfy type checking; replace with actual
    # construction when the remainder of the function is present.
