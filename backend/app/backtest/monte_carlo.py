"""Monte Carlo simulation utilities for bootstrapping equity curves and estimating confidence intervals.

This module provides functions to run a Monte‑Carlo simulation on a series of daily
returns, generating multiple possible equity paths. The results include median,
5th and 95th percentile Sharpe ratios, maximum drawdown statistics, and the
probability of achieving a positive return after the simulated horizon.
"""
...
class MonteCarloResult:
    ...
    ----------
    median_sharpe: float
        Median annualized Sharpe ratio across all simulations.
    p5_sharpe: float
    ...
    prob_positive_return: float
        Probability that a simulation ends with a positive return.
    num_simulations: int
        Number of Monte Carlo paths generated (hidden from repr).
    """
...
def _validate_inputs(
    ...
    ----------
    daily_returns : pd.Series
        Series of daily returns to be bootstrapped.
    n_simulations : int
...
def _sample_returns(
    ...
    n_simulations : int
        Number of Monte‑Carlo paths to generate.
    n_days : int
        Number of trading days in each simulated path.

    Returns
    -------
    np.ndarray
        A matrix of shape (n_simulations, n_days) containing sampled returns.
    """
...
def _calculate_equity(sampled: np.ndarray) -> np.ndarray:
    ...
    ----------
    sampled : np.ndarray
        Matrix of sampled daily returns.
...
def _calculate_max_dd(equity: np.ndarray) -> np.ndarray:
    ...
    ----------
    equity : np.ndarray
        Equity curves generated by the simulation.
...
def _calculate_sharpe(sampled: np.ndarray, risk_free_daily: float) -> np.ndarray:
    ...
    -------
    np.ndarray
        Annualized Sharpe ratio for each simulation.
    """
...
def _calculate_positive_return(equity: np.ndarray) -> int:
    ...
    -------
    int
        Number of simulations with a final equity greater than the initial capital.
    """
