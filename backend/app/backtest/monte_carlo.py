...
def monte_carlo_simulation(
    ...
    n_years: int = 3,
    risk_free_daily: float = DEFAULT_RISK_FREE_RATE / TRADING_DAYS_PER_YEAR,
) -> MonteCarloResult:
    ...
    return MonteCarloResult(
        median_sharpe=round(float(np.median(sharpes)), ROUND_DIGITS),
        p5_sharpe=round(float(np.percentile(sharpes, PERCENTILE_LOW)), ROUND_DIGITS),
        p95_sharpe=round(float(np.percentile(sharpes, PERCENTILE_HIGH)), ROUND_DIGITS),
        median_max_dd=round(float(np.median(max_dds)), ROUND_DIGITS),
        p95_max_dd=round(float(np.percentile(max_dds, PERCENTILE_HIGH)), ROUND_DIGITS),
        p5_max_dd=round(float(np.percentile(max_dds, PERCENTILE_LOW)), ROUND_DIGITS),
