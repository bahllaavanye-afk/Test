...
def monte_carlo_simulation(
    ...
    n_years : int, default 3
        Length of each simulated path in years. Must be a positive integer.
    risk_free_daily : float, default 0.05/252
        Daily risk‑free rate. Must be a real number (NaN not allowed).
    ...
    if not np.issubdtype(clean_returns.dtype, np.number):
    ...
    if not isinstance(risk_free_daily, Real):
        raise ValueError("risk_free_daily must be a real number.")
    if isinstance(risk_free_daily, float) and math.isnan(risk_free_daily):
        raise ValueError("risk_free_daily cannot be NaN.")
