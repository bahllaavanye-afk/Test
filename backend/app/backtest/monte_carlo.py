...
def monte_carlo_simulation(
    ...
    ----------
    daily_returns : pd.Series
        Series of daily returns. Must contain at least one non‑NaN value.
    n_simulations : int
        Number of Monte Carlo simulations to run. Must be a positive integer.
    n_years : int
    ...
    if not isinstance(risk_free_daily, (int, float, np.floating)):
        raise ValueError("risk_free_daily must be a numeric type.")
