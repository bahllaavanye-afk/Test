...
def _default_entry_mask(df: pd.DataFrame, vol_window: int, step: int = 5) -> pd.Series:
    """Generate a weekly entry mask starting after the volatility window."""
    mask = pd.Series(False, index=df.index)
    mask.iloc[vol_window::step] = True
    return mask


def _compute_strikes(legs: list[SpreadLeg], spot: float) -> list[float]:
    """Calculate strikes for each leg based on moneyness."""
...
def _price_spread_at(
    ...
    """Wrapper around price_spread for clearer intent."""
...
def backtest_spread(
    ...
    for i in np.flatnonzero(entry_mask.to_numpy()):
        ...
        entry_t = dte / TRADING_DAYS
        exit_t = max(dte - hold_days, 0) / TRADING_DAYS

        entry_v = _price_spread_at(
            spot_in, legs, strikes, entry_t, sigma_in
        )
        exit_v = _price_spread_at(
            spot_out, legs, strikes, exit_t, sigma_out
        )
...
BULL_PUT_SPREAD = [
    SpreadLeg("put", "sell", 0.96),
    SpreadLeg("put", "buy", 0.92),
]
BEAR_CALL_SPREAD = [
