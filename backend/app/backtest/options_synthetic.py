...
def bs_price(
    ...
    intrinsic = max(spot - strike, 0.0) if kind == OPTION_CALL else max(strike - spot, 0.0)
...
class SpreadBacktestResult:
    ...
    def summary(self) -> str:
        wr = f"{self.win_rate:.0%}" if self.win_rate is not None else DASH
        return (
...
def price_spread(
    ...
    for leg, strike in zip(legs, strikes):
        ...
        value += p if leg.side == SIDE_BUY else -p
    return value
...
def backtest_spread(
    ...
    if entry_mask is None:
        ...
        entry_mask.iloc[vol_window::WEEKLY_STEP] = True
    ...
    for i in np.flatnonzero(entry_mask.to_numpy()):
        ...
        entry_v = price_spread(spot_in, legs, strikes, dte / TRADING_DAYS, sigma_in)
        exit_v = price_spread(
            spot_out,
            legs,
            strikes,
...
IRON_CONDOR = [
    SpreadLeg(OPTION_PUT, SIDE_SELL, 0.95),
    SpreadLeg(OPTION_PUT, SIDE_BUY, 0.91),
    SpreadLeg(OPTION_CALL, SIDE_SELL, 1.05),
    SpreadLeg(OPTION_CALL, SIDE_BUY, 1.09),
]
BULL_PUT_SPREAD = [
    SpreadLeg(OPTION_PUT, SIDE_SELL, 0.96),
    SpreadLeg(OPTION_PUT, SIDE_BUY, 0.92),
]
BEAR_CALL_SPREAD = [
    SpreadLeg(OPTION_CALL, SIDE_SELL, 1.04),
    SpreadLeg(OPTION_CALL, SIDE_BUY, 1.08),
]