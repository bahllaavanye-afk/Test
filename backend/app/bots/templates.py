...
BOT_TEMPLATES: dict[str, dict] = {
    ...
    # ── Hybrids — engineered from the 2y backtest + live evidence (2026-07-06) ──
    # Recipe: winners were (a) trend-following long gamma (+$18.7k/2y), (b)
    # trend-GATED put-side premium selling (75-77% win), (c) timed entries,
    # (d) WIDE 0DTE shorts with early profit-taking (tight ones bled tails).
    # Losers: undefined risk, ATM butterflies, selling against the trend.
    "hybrid_regime_gamma": {
        "name": "Hybrid — Regime Long Gamma",
        "description": "Best-of hybrid: the 2y champion (long calls) with the 'Trendy' 50-MA gate + RSI confirm. Long 60Δ call 30 DTE only in a strong uptrend. TP 100%, stop 50%, 10-day exit.",
        "symbol": "SPY",
        "market_type": "options",
        "trigger": {"type": "schedule", "interval": "1m"},
        "conditions": [
        ...
        "condition_logic": "ALL",
        "action": {"type": "open_option_spread", "size_pct": 2.5, "take_profit_pct": 100,
                   "legs": [{"side": "buy", "option_type": "call", "delta": 0.60, "dte": 30, "ratio": 1}]},
        "exit_rules": [{"type": "take_profit", "value": 100}, {"type": "stop_loss", "value": 50},
                       {"type": "time_exit", "hours": 240}],
    },
    "hybrid_regime_theta": {
        "name": "Hybrid — Regime Theta Harvest",
        "description": "Complement to Regime Gamma: IRON Boi's 20Δ/5Δ condor structure, entered ONLY in chop (RSI 40-60 band, the Vix-range gate) with the BWB lesson of taking profits EARLY. 7 DTE, TP 25%, stop 100%.",
        "symbol": "SPY",
        "market_type": "options",
        "trigger": {"type": "schedule", "interval": "1m"},
        "conditions": [
        ...
        "condition_logic": "ALL",
        "action": {"type": "open_option_spread", "size_pct": 2.5, "take_profit_pct": 25,
                   "legs": [
                       {"side": "sell", "option_type": "call", "delta": 0.20, "dte": 7, "ratio": 1},
                       {"side": "buy", "option_type": "call", "delta": 0.05, "dte": 7, "ratio": 1},
                       {"side": "sell", "option_type": "put", "delta": 0.20, "dte": 7, "ratio": 1},
    ...
    "hybrid_put_ladder_trend": {
        "name": "Hybrid — Trend Put Ladder",
        "description": "Steamroller's daily cadence + Trendy's 50-MA gate + the best win-rate strikes (25Δ/15Δ from Short Put Spread, 77%). Daily afternoon entry, 14 DTE, put-side ONLY (call credit spreads lost fighting the uptrend). TP 50%, stop 100%.",
        "symbol": "SPY",
        "market_type": "options",
        "trigger": {"type": "schedule", "interval": "1m"},
        "conditions": [
        ...
        "condition_logic": "ALL",
        "action": {"type": "open_option_spread", "size_pct": 2.5, "take_profit_pct": 50,
                   "legs": [
                       {"side": "sell", "option_type": "put", "delta": 0.25, "dte": 14, "ratio": 1},
                       {"side": "buy", "option_type": "put", "delta": 0.15, "dte": 14, "ratio": 1}]},
        "exit_rules": [{"type": "take_profit", "value": 50}, {"type": "stop_loss", "value": 100}],
    },
    "hybrid_wide_0dte_tp25": {
        "name": "Hybrid — Wide 0DTE TP25",
        "description": "Fixes NCR-B's tail bleed (82% win, negative P&L): keep NCR-A's WIDE strikes (15Δ/5Δ), but take profit at 25% (not 50%) and go flat within 3h. Morning entry window.",
        "symbol": "SPY",
        "market_type": "options",
        "trigger": {"type": "schedule", "interval": "1m"},
        "conditions": [
        ...
        "action": {"type": "open_option_spread", "size_pct": 2.5, "take_profit_pct": 25,
                   "legs": [
                       ...
                       {"side": "sell", "option_type": "put", "delta": 0.15, "dte": 0, "ratio": 1},
                       {"side": "buy", "option_type": "put", "delta": 0.05, "dte": 0, "ratio": 1}]},
        "exit_rules": [{"type": "take_profit", "value": 25}, {"type": "stop_loss", "value": 150},
                       {"type": "time_exit", "hours": 3}],
    },
    "hybrid_morning_momentum": {
        "name": "Hybrid — Morning Momentum Daytrade",
        "description": "TD.NPDT's day-trade discipline + IB@9:45's timing edge + dual confirmation (20-MA AND RSI>60). Long 60Δ call 1 DTE in the 9:45-10:00 window, flat by close. TP 60%, stop 40%.",
        "symbol": "QQQ",
        "market_type": "options",
        "trigger": {"type": "schedule", "interval": "1m"},
        "conditions": [
            {"type": "time_window", "start_time": "13:45", "end_time": "14:00"},
            {"type": "price_vs_ma", "ma_period": 20, "operator": ">", "ma_type": "sma"},
            {"type": "indicator", "indicator": "rsi", "period": 14, "operator": ">", "value": 60},
            {"type": "no_position"},
        ],
        "condition_logic": "ALL",
        "action": {"type": "open_option_spread", "size_pct": 2.5, "take_profit_pct": 60,
                   "legs": [{"side": "buy", "option_type": "call", "delta": 0.60, "dte": 1, "ratio": 1}]},
        "exit_rules": [{"type": "take_profit", "value": 60}, {"type": "stop_loss", "value": 40},
                       {"type": "time_exit", "hours": 6}],
    },
