...
def alpaca_headers() -> dict[str, str]:
    ...
    Raises:
        ValueError: If the Alpaca API key or secret key is missing, not a string,
        or an empty string.
    """
    ...
    if not isinstance(api_key, str) or not api_key:
        raise ValueError("Invalid Alpaca API key: must be a non-empty string.")
    if not isinstance(secret_key, str) or not secret_key:
        raise ValueError("Invalid Alpaca secret key: must be a non-empty string.")
