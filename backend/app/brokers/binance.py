...
class BinanceBroker(AbstractBroker):
    def __init__(self, api_key: str, secret: str, testnet: bool = True):
        ...
        if not CCXT_AVAILABLE:
            raise BrokerError("CCXT library is not available; Binance broker cannot be instantiated")
    ...
    async def place_order(self, request: OrderRequest) -> OrderResult:
        ...
        if not isinstance(request.quantity, Number) or request.quantity <= 0:
            raise ValueError("quantity must be a positive number")
        if request.order_type == "limit":
            if request.limit_price is None:
                raise ValueError("limit_price must be provided for limit orders")
            if not isinstance(request.limit_price, Number) or request.limit_price <= 0:
