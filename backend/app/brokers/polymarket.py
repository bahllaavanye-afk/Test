...
class PolymarketBroker(AbstractBroker):
    ...
    async def place_order(self, request: OrderRequest) -> OrderResult:
        ...
        if request.limit_price is not None and not isinstance(request.limit_price, (int, float)):
            raise ValueError("request.limit_price must be numeric or None")
    ...
    async def get_historical(self, symbol: str, interval: str = "1d", limit: int = 500) -> list[dict]:
        if not isinstance(symbol, str) or not symbol:
            raise ValueError("symbol must be a non‑empty string")
        allowed_intervals = {"1m", "5m", "15m", "1h", "1d", "1w"}
