...
class PolymarketBroker(AbstractBroker):
    ...
    async def get_quote(self, symbol: str) -> QuoteResult:
        """Retrieve the best bid/ask and compute a mid price."""
        ob = await self.get_order_book(symbol)
        best_bid, best_ask = self._extract_best_prices(ob)
        return QuoteResult(
            symbol=symbol,
            bid=best_bid,
            ask=best_ask,
    ...
    def _extract_best_prices(self, order_book: dict) -> tuple[float, float]:
        """Extract the best bid and ask from an order book."""
