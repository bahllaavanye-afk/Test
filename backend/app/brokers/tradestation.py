...
class TradeStationBroker(AbstractBroker):
    ...
    async def place_order(self, request: OrderRequest) -> OrderResult:
        """Place a single‑leg order and return a parsed ``OrderResult``."""
    ...
    def _build_order_body(self, request: OrderRequest) -> dict:
        """Construct the JSON payload required by TradeStation's order endpoint."""
        body = {
    ...
    def _parse_order_response(self, data: dict) -> OrderResult:
        """Translate TradeStation's response JSON into an ``OrderResult``."""
    ...
    def build_option_order_body(
        ...
        *,
        ...
        legs: list[dict]
            Each dict must contain ``Symbol`` and ``TradeAction`` fields.
        quantity: int, default 1
            Number of contracts (per leg) to trade.
        ...
        legs_body = [
            {
                ...
                "Quantity": str(int(leg.get("Quantity", quantity))),
            }
            for leg in legs
        ]
