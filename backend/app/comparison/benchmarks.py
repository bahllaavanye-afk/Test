...
def _get_cached_result(key: tuple[date, date]) -> dict[str, List[dict]] | None:
    """Retrieve a shallow copy of a cached benchmark result, if present."""
...
async def _fetch_all_series(
    ...
    """
    Fetch series for each ticker concurrently.
    Returns a dict mapping ticker to a non‑empty pd.Series.
    """
...
def _process_all_weather(
    ...
    """Calculate the monthly rebalanced All Weather portfolio."""
...
async def fetch_benchmark_curves(start: date, end: date) -> dict[str, List[dict]]:
    """Return benchmark curves normalized to 100 at the start date."""
