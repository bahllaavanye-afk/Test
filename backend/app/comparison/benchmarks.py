...
async def fetch_benchmark_curves(start: date, end: date) -> dict[str, List[dict]]:
    ...
    ----------
    start : datetime.date
        The start date (inclusive) for the benchmark data.
    end : datetime.date
        The end date (exclusive) for the benchmark data.

    Raises
    ------
    ValueError
        If `start` or `end` are not `datetime.date` instances, or if `start` is not
        earlier than `end`.
    """
    # Input validation
