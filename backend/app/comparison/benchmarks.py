...
def _get_cached_result(cache_key: tuple[date, date]) -> dict[str, List[dict]] | None:
    """Retrieve a shallow copy of cached data if present."""
...
def _process_gather_results(
    tickers: List[str], raw_series: List[pd.Series | Exception]
) -> List[pd.Series]:
    """Replace any Exception entries with an empty Series and log the error."""
    ...
    for ticker, result in zip(tickers, raw_series):
        if isinstance(result, Exception):
            ...
            processed.append(pd.Series(dtype=float))
        else:
...
def _build_individual_results(
    closes_dict: Mapping[str, pd.Series]
) -> dict[str, List[dict]]:
    """Normalize each benchmark series to 100 at the start date."""
    result: dict[str, List[dict]] = {}
    for ticker, series in closes_dict.items():
...
def _build_all_weather_result(
    ...
    """Calculate the monthly‑rebalanced All Weather portfolio equity curve."""
    ...
    weights = weights / weights.sum()  # Renormalize in case of missing tickers
...
async def fetch_benchmark_curves(start: date, end: date) -> dict[str, List[dict]]:
    ...
    series_list = _process_gather_results(all_tickers, raw_series)
    ...
    result = _build_individual_results(closes_dict)

    aw_result = _build_all_weather_result(closes_dict)
    if aw_result:
        result["ALL_WEATHER"] = aw_result
