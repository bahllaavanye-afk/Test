...
class StrategyComparisonEngine:
    async def run_comparison(
        ...
        """
        Run a side‑by‑side backtest of a manual and an ML‑enhanced signal series.

        Raises:
            ValueError: If any input does not meet the expected type or logical constraints.
        """
        # ---- Input validation ----
        ...
        # Ensure signal series align with price series (same length and index)
        if len(manual_signals) != len(prices):
            raise ValueError("manual_signals length must match prices length.")
        if len(ml_signals) != len(prices):
            raise ValueError("ml_signals length must match prices length.")
        if not manual_signals.index.equals(prices.index):
            raise ValueError("manual_signals index must match prices index.")
        if not ml_signals.index.equals(prices.index):
            raise ValueError("ml_signals index must match prices index.")
