...
MIN_TTEST_SAMPLE_SIZE: int = 10
IMPROVEMENT_THRESHOLD: float = 0.1
SIGNIFICANCE_LEVEL: float = 0.05
SHARPE_ROUND_PRECISION: int = 4
PVALUE_ROUND_PRECISION: int = 4
PVALUE_FULL_PRECISION: int = 6
WINNER_NEITHER: str = "neither"
LOG_MSG_COMPARISON_COMPLETE: str = "Comparison complete"
...
class StrategyComparisonEngine:
    async def run_comparison(
        ...
        if min_len > MIN_TTEST_SAMPLE_SIZE:
        ...
        logger.info(
            LOG_MSG_COMPARISON_COMPLETE,
            strategy=strategy_name,
            manual_sharpe=manual_metrics.sharpe,
            ml_sharpe=ml_metrics.sharpe,
            p_value=round(p_val, PVALUE_ROUND_PRECISION),
        )
        ...
        return ComparisonResult(
            ...
            ml_improvement_sharpe=round(improvement, SHARPE_ROUND_PRECISION),
            t_statistic=round(float(t_stat), SHARPE_ROUND_PRECISION),
            p_value=round(float(p_val), PVALUE_FULL_PRECISION),
