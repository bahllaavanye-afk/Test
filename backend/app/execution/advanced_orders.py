    ...
    async def execute(self, config: BracketOrderConfig) -> OrderResult:
        ...
        try:
            ...
            if not await self._price_within_tolerance(config.entry, market_price):
                ...
                logger.info(
                    "BracketOrder completed (price tolerance exceeded)",
        ...
        if entry_result.status not in ("filled", "partially_filled"):
            ...
            duration = time.monotonic() - start_time
            logger.info(
                "BracketOrder completed (entry not filled)",
                signal_count=signal_count,
                execution_time_seconds=duration,
                pnl=None,
            )
        ...
        if tp_price <= sl_price:
            ...
            duration = time.monotonic() - start_time
            logger.info(
                "BracketOrder completed (invalid TP/SL)",
                signal_count=signal_count,
                execution_time_seconds=duration,
                pnl=None,
            )
        ...
        signal_count += 2  # TP and SL signals
...
class OCOOrder:
    ...
    async def execute(self, order_a: OrderRequest, order_b: OrderRequest) -> OrderResult:
        start_time = time.monotonic()
        signal_count = 2  # two opposing orders
...
class TrailingStop:
    ...
    async def execute(self, request: OrderRequest, trail_pct: float = 0.05) -> OrderResult:
        start_time = time.monotonic()
        signal_count = 1  # initial order

        if request.side == "sell":
            # Placeholder for sell-side logic
            pass
        else:
            # Placeholder for buy-side logic
            pass

        # As the detailed implementation is omitted, we log completion with available metrics.
        duration = time.monotonic() - start_time
        logger.info(
            "TrailingStop execution completed",
            signal_count=signal_count,
            execution_time_seconds=duration,
            pnl=None,
        )
        # Return a generic rejected result to keep type consistency; real implementation should replace this.
        return OrderResult(
            broker_order_id="",
            status="rejected",
            avg_fill_price=None,
