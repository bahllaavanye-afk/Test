...
class SlippageTracker:
    ...
    async def record_fill(
        ...
        execution_duration_seconds = (
            (fill_time - submit_time).total_seconds() if submit_time else None
        )

        is_cost_bps = self._compute_is_cost_bps(request, result, arrival_price)
        vwap_shortfall_bps = self._compute_vwap_shortfall_bps(request, result, period_vwap)
        slippage_bps = self._compute_slippage_bps(request, result, signal_price)
        ...
        if slippage_bps is not None:
            ...
            await self._notify_and_track(request, result, slippage_bps)
    ...
    def _compute_is_cost_bps(
        ...
        if arrival_price is None or arrival_price <= 0:
            return None
        if request.side == "buy":
            return (result.avg_fill_price - arrival_price) / arrival_price * 10_000
        return (arrival_price - result.avg_fill_price) / arrival_price * 10_000

    def _compute_vwap_shortfall_bps(
        self,
        request: OrderRequest,
        result: OrderResult,
        period_vwap: float | None,
    ) -> Optional[float]:
        if period_vwap is None or period_vwap <= 0:
            return None
        if request.side == "buy":
            return (result.avg_fill_price - period_vwap) / period_vwap * 10_000
        return (period_vwap - result.avg_fill_price) / period_vwap * 10_000
    ...
    def _compute_slippage_bps(
        ...
        if signal_price is None:
            return None
        if request.side == "buy":
            return (result.avg_fill_price - signal_price) / signal_price * 10_000
        return (signal_price - result.avg_fill_price) / signal_price * 10_000
    ...
    def _log_slippage(
        ...
        fill_price: float,
        slippage_bps: float,
        is_cost_bps: Optional[float],
        vwap_shortfall_bps: Optional[float],
