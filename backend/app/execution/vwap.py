...
class VWAPExecution:
    ...
    async def execute(self, request: OrderRequest) -> OrderResult:
        ...
        # Compute P&L if reference price is available
        pnl = None
        if avg_price is not None and getattr(request, "price", None) is not None:
            # Positive P&L when execution price is better than the reference price
            pnl = (request.price - avg_price) * total_filled
