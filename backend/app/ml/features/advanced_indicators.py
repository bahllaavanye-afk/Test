...
def _log_metrics(func):
    """Decorator that logs execution time, signal count, and P&L (if applicable)."""
    ...
    def wrapper(*args, **kwargs):
        ...
        # Determine signal count and P&L based on result type
        signal_count = 0
        pnl = None

        if isinstance(result, pd.Series):
            signal_count = int(result.notna().sum())
            if np.issubdtype(result.dtype, np.number):
                pnl = float(result.sum())
        elif isinstance(result, pd.DataFrame):
            signal_count = int(result.notna().sum().sum())
            numeric_df = result.select_dtypes(include=[np.number])
            if not numeric_df.empty:
                pnl = float(numeric_df.sum().sum())
...
def roll_spread(close: pd.Series, window: int = 21) -> pd.Series:
    """
    Roll (1984) spread estimator: 2 * sqrt( (sigma^2) - (sigma_f^2) )
    where sigma is the variance of the observed price series and sigma_f
    is the variance of the futures price series (approximated via close).
    """
    # Placeholder implementation – actual roll spread requires futures data.
    # Here we approximate using the close series only.
    variance = close.rolling(window).var(ddof=1)
    result = 2 * np.sqrt(np.maximum(variance, 0))
    result.name = "roll_spread"
    return result

# Note: The remainder of the original file (including add_advanced_features
# and ADVANCED_FEATURE_COLS) is unchanged and follows after this point.