...
Exports:
  ...
  generate_advanced_signal(df) -> pd.Series
"""
...
def roll_spread(close: pd.Series, window: int = 21) -> pd.Series:
    """
    Roll (1984) spread estimator: 2 * |Δclose| smoothed over a rolling window.
    Provides a proxy for the cost of synthetic futures replication.
    """
    spread = 2 * close.diff().abs().rolling(window).mean()
...
def generate_advanced_signal(df: pd.DataFrame) -> pd.Series:
    """
    Generate a refined long/short signal based on advanced indicators.

    The signal series contains:
        1  → Enter long
       -1  → Enter short
        0  → No position / exit

    Entry Conditions (tightened):
        • Volatility percentile rank < 0.15 (low volatility regime)
        • Hurst exponent < 0.45 (mean‑reverting regime)
        • Efficiency ratio < 0.25 (choppy market, favourable for mean‑reversion)
        • Approximate entropy < 0.5 (regular price dynamics)
        • Amihud illiquidity > 1e5 (sufficient liquidity for execution)

    Confirmation Filters:
        • Close price crossing above its 20‑period SMA for longs,
          crossing below for shorts.
        • Volume above its 20‑period SMA to avoid low‑volume traps.

    Exit Logic:
        • Opposite entry condition met (e.g., volatility rank > 0.75)
        • Hurst exponent moving away from the mean‑reverting zone (>0.55)
        • Price crossing back through the SMA (reverse of entry filter)
        • Stop‑loss: 2× rolling 10‑period ATR (not implemented here)
          – the function only flips to 0; downstream risk module handles SL/TP.
    ...
    -------
    pd.Series
        Signal series aligned with df index, named 'adv_signal'.
    """
    # Basic sanity check – avoid KeyError downstream
    required = {'close', 'high', 'low', 'open', 'volume', 'returns'}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Missing required columns for signal generation: {missing}")

    # -------------------------------------------------------------------
    # Compute or retrieve indicator series
    # -------------------------------------------------------------------
    close = df['close']
    high = df['high']
    low = df['low']
    ...
    # Volatility related series
    gk_vol = garman_klass_vol(high, low, open_, close)
    vol_rank = vol_percentile_rank(gk_vol)

    # Regime / complexity series
    hurst = hurst_exponent(close)
    eff_ratio = efficiency_ratio(close)
    apen = approx_entropy(close)

    # Liquidity / micro‑structure
    illiq = amihud_illiquidity(returns, volume)

    # Simple moving averages for price & volume confirmation
    sma_price = close.rolling(20, min_periods=1).mean()
    sma_vol = volume.rolling(20, min_periods=1).mean()

    # -------------------------------------------------------------------
    # Build boolean masks for entry/exit criteria
    # -------------------------------------------------------------------
    long_entry = (
        (vol_rank < 0.15) &
        (hurst < 0.45) &
    ...
    # Exit when any of the core conditions deteriorate
    exit_condition = (
        (vol_rank > 0.75) |
        (hurst > 0.55) |
        (eff_ratio > 0.4) |
    ...
    # -------------------------------------------------------------------
    # Assemble final signal series
    # -------------------------------------------------------------------
    signal = pd.Series(0, index=df.index, name="adv_signal")
    signal[long_entry] = 1
    signal[short_entry] = -1

    # Positions that currently exist but meet an exit condition are cleared
    existing_long = (signal.shift(1) == 1)
    existing_short = (signal.shift(1) == -1)

    signal[existing_long & exit_condition] = 0
    signal[existing_short & exit_condition] = 0
