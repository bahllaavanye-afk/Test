...
def _monitor(func: Callable[..., pd.Series]) -> Callable[..., pd.Series]:
    """Decorator that logs execution time, signal count, and placeholder P&L."""
    def wrapper(*args: Any, **kwargs: Any) -> pd.Series:
        start = time.time()
        result = func(*args, **kwargs)
        elapsed = time.time() - start
        # Count non‑NaN signals produced by the indicator
        signal_count = int(result.count()) if isinstance(result, pd.Series) else 0
        # Placeholder for P&L – not computed in this module
        pnl = None
        _logger.info(
            "Indicator %s | exec_time=%.6fs | signal_count=%d | pnl=%s",
            func.__name__,
            elapsed,
            signal_count,
            pnl,
...
def roll_spread(close: pd.Series, window: int = 21) -> pd.Series:
    """
    Roll (1984) spread estimator: 2 * sqrt(rolling variance of log returns).
    """
    log_ret = np.log(close / close.shift(1))
    var = log_ret.rolling(window).var(ddof=1)
    result = 2 * np.sqrt(var)
    result.name = "roll_spread"
    return result

# Additional indicators would follow the same pattern...