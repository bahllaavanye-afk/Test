...
class AlphaDecayTracker:
    ...
    def compute_ic_profile(
        ...
        Raises:
            ...
            ValueError: If ``prices`` lacks a ``close`` column.
        """
        if not isinstance(signals, pd.Series):
            logger.error(
                "Invalid type for signals",
        ...
        if not isinstance(prices, pd.DataFrame):
            logger.error(
                "Invalid type for prices",
                extra={"expected_type": "pd.DataFrame", "actual_type": type(prices).__name__},
            )
        ...
        for h in self.HORIZONS:
            try:
                ...
                logger.exception(
                    "Failed to compute IC for horizon %s",
                    h,
        ...
        if len(ics) < 2:
            logger.info(
                "Insufficient IC points for decay fitting",
                extra={"strategy_name": strategy_name, "ic_points": len(ics)},
            )
            return DecayProfile(
        ...
        try:
            ...
            logger.exception(
                "Unexpected error during curve fitting",
                extra={"exception_type": type(exc).__name__},
            )
            ic_0 = float(ic_arr[0]) if ic_arr.size > 0 else 0.0
            half_life = float("inf")
    ...
    def scale_confidence(
        ...
        try:
            ...
            adjusted = float(base_confidence * max(decay, 0.0))
            return adjusted
        except Exception as exc:
            logger.exception(
                "Error computing scaled confidence",
                extra={"exception_type": type(exc).__name__, "staleness_hours": staleness_hours},
            )
            # Fallback to unscaled confidence on unexpected error
            return float(base_confidence)