...
class AlphaDecayTracker:
    ...
    def compute_ic_profile(
        ...
        for h in self.HORIZONS:
            ...
            used_signal_count += len(s)
            forward_returns.extend(r.tolist())
        ...
        if len(ics) < 2:
            ...
            logger.info(
                ...
                extra={
                    ...
                    "signal_count_total": total_signal_count,
                    "signal_count_used": used_signal_count,
                    "horizons_measured": list(ics.keys()),
                    "ic_0": profile.ic_0,
                    "half_life_hours": profile.half_life_hours,
                    "avg_forward_return": np.mean(forward_returns) if forward_returns else float("nan"),
    ...
    def scale_confidence(
        ...
        elapsed = time.time() - start_time
        logger.info(
            "AlphaDecay scale_confidence executed",
            extra={
