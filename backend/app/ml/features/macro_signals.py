"""
Macro signal utilities for free data sources.

This module provides asynchronous functions to fetch key macroeconomic indicators
from the FRED API and Reddit sentiment from Apewisdom. All data sources are free
and do not require an API key for basic usage.
"""
...
async def _fred_latest(series_id: str, api_key: str = "DEMO_KEY") -> Optional[float]:
    ...
    ----------
    series_id: str
        The FRED series identifier (e.g., ``T10Y2Y`` for the 10‑year/2‑year yield spread).
    api_key: str, optional
        API key for FRED. ``DEMO_KEY`` provides up to 500 requests per day and
...
async def get_macro_snapshot() -> Dict[str, Any]:
    """
    Fetch a snapshot of core macroeconomic indicators.

    The function queries several free FRED series in parallel and derives a set
    of simple signals and an aggregate macro score.

    Returns
    -------
    dict
        A dictionary containing raw indicator values, derived signals, a macro
        score (‑3 to +3), a textual macro bias, and the UTC timestamp of retrieval.
    """
...
async def get_reddit_sentiment(tickers: List[str] | None = None) -> Dict[str, Any]:
    ...
    tickers: list[str] | None, optional
        If provided, filter the results to only include these tickers (case‑insensitive).

    Returns
    -------
    dict
        Contains up to 20 result entries, a fetch timestamp, and the data source.
        On failure, an ``error`` key is included with a brief description.
    """
    try:
        async with aiohttp.ClientSession() as sess:
...
async def get_macro_snapshot_cached() -> Dict[str, Any]:
    """
    Return a cached macro snapshot if it is recent enough.

    The cache expires after ``MACRO_CACHE_SECONDS`` (default 5 minutes). When
    expired, a fresh snapshot is fetched via :func:`get_macro_snapshot`.
