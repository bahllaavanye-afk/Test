...
import aiohttp
from aiohttp import ClientError, ClientResponseError, ClientConnectionError, ContentTypeError
...
async def _fred_latest(series_id: str, api_key: str = "DEMO_KEY") -> Optional[float]:
    ...
    try:
        ...
        logger.error(
            "Network or response error while fetching FRED data",
            extra={"series_id": series_id, "error": str(e)},
        )
    except asyncio.TimeoutError as e:
        logger.error(
            "Timeout fetching FRED data",
            extra={"series_id": series_id, "error": str(e)},
        )
    except Exception as e:  # pragma: no cover
        logger.exception(
            "Unexpected error during FRED fetch",
...
async def get_macro_snapshot() -> Dict[str, Any]:
    ...
    for idx, result in enumerate(results):
        if isinstance(result, Exception):
            logger.error(
                "Error in macro fetch task",
                extra={"task_index": idx, "error": str(result)},
            )
...
async def get_reddit_sentiment(tickers: List[str] | None = None) -> Dict[str, Any]:
    ...
    try:
        async with aiohttp.ClientSession() as sess:
            async with sess.get(
                APEWISDOM_URL, timeout=aiohttp.ClientTimeout(total=5)
            ) as resp:
                if resp.status != 200:
                    logger.error(
        ...
        logger.error(
            "Network or response error while fetching Reddit sentiment",
            extra={"error": str(e)},
        )
        return {"error": str(e), "results": []}
    except asyncio.TimeoutError as e:
        logger.error(
            "Timeout fetching Reddit sentiment",
            extra={"error": str(e)},
        )
        return {"error": "timeout", "results": []}
    except Exception as e:  # pragma: no cover
        logger.exception(
            "Unexpected error during Reddit sentiment fetch",
        )
        return {"error": str(e), "results": []}
