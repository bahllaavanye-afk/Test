...
class OrderBookFeatures:
    ...
    def compute_imbalance(
        ...
        if not bids or not asks:
            _logger.info(
                "compute_imbalance: empty bids or asks",
                extra={"signal_count": 0, "execution_time_ms": (time.perf_counter() - start) * 1e3},
            )
            return 0.0
        ...
        if total <= 0.0:
            _logger.info(
                "compute_imbalance: zero total volume",
                extra={"signal_count": 0, "execution_time_ms": (time.perf_counter() - start) * 1e3},
            )
            return 0.0
        result = float((bid_vol - ask_vol) / total)
        _logger.info(
            "compute_imbalance computed",
            extra={"signal_count": 1, "execution_time_ms": (time.perf_counter() - start) * 1e3},
        )
        return result
    ...
    def compute_spread_bps(self, best_bid: float, best_ask: float) -> float:
        ...
        if best_bid <= 0.0 or best_ask <= 0.0 or best_ask <= best_bid:
            _logger.info(
                "compute_spread_bps: invalid inputs",
                extra={"signal_count": 0, "execution_time_ms": (time.perf_counter() - start) * 1e3},
            )
            return 0.0
    ...
    def compute_depth_ratio(
        ...
        if not bids or not asks:
            _logger.info(
                "compute_depth_ratio: empty bids or asks",
                extra={"signal_count": 0, "execution_time_ms": (time.perf_counter() - start) * 1e3},
            )
            return 1.0
        ...
        if best_ask_size <= 0.0:
            _logger.info(
                "compute_depth_ratio: non-positive ask size",
                extra={"signal_count": 0, "execution_time_ms": (time.perf_counter() - start) * 1e3},
            )
            return 1.0
        result = float(best_bid_size / best_ask_size)
        _logger.info(
            "compute_depth_ratio computed",
            extra={"signal_count": 1, "execution_time_ms": (time.perf_counter() - start) * 1e3},
        )
        return result
    ...
    def compute_pin_proxy(self, buy_volume: float, sell_volume: float) -> float:
        ...
        if total <= 0.0:
            _logger.info(
                "compute_pin_proxy: zero total volume",
                extra={"signal_count": 0, "execution_time_ms": (time.perf_counter() - start) * 1e3},
            )
            return 0.0
        result = float(abs(buy_volume - sell_volume) / total)
        _logger.info(
            "compute_pin_proxy computed",
            extra={"signal_count": 1, "execution_time_ms": (time.perf_counter() - start) * 1e3},
        )
        return result
    ...
    def compute_kyle_lambda(
        ...
        try:
            ...
            if var_vol < 1e-12:
                _logger.info(
                    "compute_kyle_lambda: variance too low",
                    extra={"signal_count": 0, "execution_time_ms": (time.perf_counter() - start) * 1e3},
                )
                return 0.0
            ...
            _logger.info(
                "compute_kyle_lambda computed",
                extra={"signal_count": 1, "execution_time_ms": (time.perf_counter() - start) * 1e3},
            )
            return lam
        except Exception:
            _logger.exception(
                "compute_kyle_lambda: exception during computation",
                extra={"signal_count": 0, "execution_time_ms": (time.perf_counter() - start) * 1e3},
            )
            return 0.0
    ...
    def features_from_snapshot(
        ...
        _logger.info(
            ...
            extra={"signal_count": len(result), "execution_time_ms": elapsed_ms, "pnl": None},
        )
        return result
...
def add_microstructure_features(
    ...
    _logger.info(
        ...
        extra={"signal_count": len(df), "execution_time_ms": elapsed_ms, "pnl": None},
    )
    return df
