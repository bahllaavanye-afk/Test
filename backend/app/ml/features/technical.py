...
# Volatility rolling windows (in days)
VOLATILITY_PERIODS = (5, 21, 63)
...
def add_technical_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if obv is not None:
        try:
            df["obv_change"] = obv.pct_change(OBV_CHANGE_PERIOD).fillna(0)
        except Exception as exc:  # pragma: no cover
            _logger.error("Failed to compute OBV change", exc_info=exc)

    # --- Volume ratio ---
    try:
        vol_ma = volume.rolling(VOLUME_RATIO_PERIOD).mean()
