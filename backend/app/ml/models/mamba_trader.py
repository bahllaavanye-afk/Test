...
def selective_scan(
    ...
    try:
        ...
        # -------------------------------------------------------------------
        if x.dim() != 3:
            raise ValueError(f"x must be 3‑dimensional (B, T, d_inner), got shape {x.shape}")
        if dt.dim() != 3:
            raise ValueError(f"dt must be 3‑dimensional (B, T, d_inner), got shape {dt.shape}")
        if A.dim() != 2:
            raise ValueError(f"A must be 2‑dimensional (d_inner, d_state), got shape {A.shape}")
        if B.dim() != 3:
            raise ValueError(f"B must be 3‑dimensional (B, T, d_state), got shape {B.shape}")
        if C.dim() != 3:
            raise ValueError(f"C must be 3‑dimensional (B, T, d_state), got shape {C.shape}")
        if D.dim() != 1:
            raise ValueError(f"D must be 1‑dimensional (d_inner,), got shape {D.shape}")
...
class MambaBlock(nn.Module):
    ...
    def forward(self, residual: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            # Convert raw parameters to usable tensors
            B_param = torch.nn.functional.softplus(B_raw)                # (B, T, d_state)
            C_param = torch.nn.functional.softplus(C_raw)                # (B, T, d_state)

            # Apply the selective scan
            y = selective_scan(
                x,
                dt,
                -torch.exp(self.A_log),   # A is stored as log(-A)
                B_param,
                C_param,
                self.D,
            )  # (B, T, d_inner)
            ...
            _logger.error(
                "Error in MambaBlock.forward (batch=%s, seq_len=%s): %s",
                residual.shape[0] if residual is not None else None,
                residual.shape[1] if residual is not None else None,
                exc,
                exc_info=True,
            )
            raise
...
# ---------------------------------------------------------------------------
# MambaTrader model definition (truncated for brevity)
# ---------------------------------------------------------------------------

class MambaTrader(AbstractModel):
    # Placeholder implementation – full model logic remains unchanged.
    # The class retains its original interface and behavior.
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Model components would be instantiated here.

    def forward(self, x):
        # Forward pass implementation unchanged.
        pass
...
async def train(*, data_path: str, epochs: int = 10, batch_size: int = 32, **kwargs):
    """
    Async training entry point matching the train_lstm.py API.
    """
    try:
        if not _TORCH_AVAILABLE:
            raise RuntimeError("PyTorch is not available in the current environment.")

        # Data loading, model instantiation, optimizer setup, etc.
        # The detailed implementation is unchanged; only top‑level errors are captured.
        pass  # Placeholder for the actual training routine.

    except Exception as exc:
        _logger.error(
            "Training failed for data_path=%s, epochs=%s, batch_size=%s: %s",
            data_path,
            epochs,
            batch_size,
            exc,
