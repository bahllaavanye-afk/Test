...
class InferenceLog(Base):
    ...
    def __init__(
        ...
        *,
        ...
        """
        Create a new InferenceLog record with validation.
        ...
        # Prediction validation: must be in [0, 1]
        ...
        # Signal validation
        if not isinstance(signal, str) or signal not in self._VALID_SIGNALS:
            raise ValueError(f"signal must be one of {sorted(self._VALID_SIGNALS)}")

        # Confidence validation: derived metric, should be in [0, 1]
        ...
        # Latency validation: non‑negative number
        ...
        # AB group validation
        if not isinstance(ab_group, str) or ab_group not in self._VALID_AB_GROUPS:
            raise ValueError(f"ab_group must be one of {sorted(self._VALID_AB_GROUPS)}")

        # Optional fields validation
        if actual_return is not None and not isinstance(actual_return, (float, int)):
            raise ValueError("actual_return must be numeric if provided")
