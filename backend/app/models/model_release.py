...
class ModelRelease(Base, TimestampMixin):
    ...
    # JSON blobs – deferred loading to avoid unnecessary data transfer
    model_params: Mapped[dict] = deferred(mapped_column(JSON, default=dict))
    training_config: Mapped[dict] = deferred(mapped_column(JSON, default=dict))
    train_metrics: Mapped[dict] = deferred(mapped_column(JSON, default=dict))
    live_metrics: Mapped[dict] = deferred(mapped_column(JSON, default=dict))
    # Serving state
    ...
    # Free-text notes from the creator – deferred as it can be large
