...
def estimate_recovery(
    ...
    if any(
        not isinstance(r, numbers.Real) or not np.isfinite(r)
        for r in returns
    ):
        raise ValueError("All elements in `returns` must be finite numeric values.")
    if not isinstance(current_drawdown, numbers.Real) or not np.isfinite(current_drawdown):
        raise ValueError("`current_drawdown` must be a finite numeric value.")
