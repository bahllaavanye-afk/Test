...
def estimate_recovery(
    ...
    if not isinstance(current_drawdown, (int, float, np.floating, np.integer)):
        raise ValueError("`current_drawdown` must be a numeric type.")
    if current_drawdown <= 0:
        raise ValueError("`current_drawdown` must be positive (greater than 0).")
