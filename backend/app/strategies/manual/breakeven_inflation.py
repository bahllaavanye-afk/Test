...
_unguarded_analyze = BreakevenInflationStrategy.analyze
...
async def _failsoft_analyze(self, data, symbol: str = "SPY"):
    try:
        ...
        print(f"breakeven_inflation: analyze fail-soft -> None ({type(exc).__name__}: {str(exc)[:80]})")
        return None


BreakevenInflationStrategy.analyze = _failsoft_analyze
