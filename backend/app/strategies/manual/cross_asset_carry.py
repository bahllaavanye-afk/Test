...
class CrossAssetCarryStrategy(AbstractStrategy):
    ...
    ENTRY_THRESHOLD = 0.60   # tightened entry
    EXIT_THRESHOLD = 0.15    # tighter exit
    ...
    async def _fetch_recent_return(self, symbol: str, days: int) -> float | None:
        """Fetch recent N‑day total return for a confirmation filter."""
        ...
        try:
            ...
            if len(bars) < days:
                return None
            closes = [float(b["c"]) for b in bars]
            return float(closes[-1] / closes[-days] - 1.0)
        except Exception:
            return None
    ...
    async def analyze(self, data: pd.DataFrame, symbol: str) -> Signal | None:
        ...
        # --------------------------------------------------------------------
        # 1️⃣ Fetch 12‑month returns for all ETFs concurrently
        # --------------------------------------------------------------------
        ...
        # Must have at least one high‑carry and one low‑carry equity, plus bond pair
        required = {"SCHD", "ARKK", "TLT", "SHY"}
        if not required.issubset(ret_map):
            return None

        # --------------------------------------------------------------------
        # 2️⃣ Compute raw spread signals
        # --------------------------------------------------------------------
        ...
        # --------------------------------------------------------------------
        # 5️⃣ Confirmation filters
        #    • Both component normals must exceed a modest magnitude
        #    • Recent short‑term price momentum must align with the intended side
        # --------------------------------------------------------------------
        magnitude_ok = (
            abs(equity_carry_norm) >= 0.2 and abs(bond_carry_norm) >= 0.2
        )
        if not magnitude_ok:
            return None

        side = "buy" if combined > 0 else "sell"
        # Choose the primary ETF for the confirmation check
        primary_etf = "SCHD" if side == "buy" else "ARKK"
        recent_ret = await self._fetch_recent_return(
            primary_etf, self.CONFIRM_LOOKBACK_DAYS
        )
        if recent_ret is None:
            return None
        momentum_ok = recent_ret > 0 if side == "buy" else recent_ret < 0
        if not momentum_ok:
            return None
        ...
        if combined >= self.ENTRY_THRESHOLD:
            # Strong signal – open or reinforce position
            confidence = min(abs(combined), 1.0)
            trade_symbol = primary_etf
            # Allow caller to override with a specific symbol from the universe
            ...
            return Signal(
                ...
                metadata={
                    ...
                    "recent_return": round(recent_ret, 4),
                    "academic_ref": "Koijen et al. (2018)",
                },
            )
        elif abs(combined) <= self.EXIT_THRESHOLD:
            # Weakening signal – signal exit/flatten position
            return Signal(
                symbol=primary_etf,
                side="exit",
                ...
                risk_bucket=self.risk_bucket,
                metadata={
                    "reason": "combined_carry_below_exit_threshold",
                    "combined_carry": round(combined, 4),
                    "academic_ref": "Koijen et al. (2018)",
                },
            )
        elif combined <= self.STOP_THRESHOLD:
            # Extreme reversal – aggressive stop out
            return Signal(
                symbol=primary_etf,
                side="exit",
                confidence=1.0,
                ...
                risk_bucket=self.risk_bucket,
                metadata={
                    "reason": "stop_threshold_breached",
                    "combined_carry": round(combined, 4),
                    "academic_ref": "Koijen et al. (2018)",
                },
            )
