...
_unguarded_analyze = DollarCarryStrategy.analyze
...
async def _failsoft_analyze(self, data, symbol: str = "SPY"):
    try:
        ...
        print(f"dollar_carry: analyze fail-soft -> None ({type(exc).__name__}: {str(exc)[:80]})")
        return None


DollarCarryStrategy.analyze = _failsoft_analyze
