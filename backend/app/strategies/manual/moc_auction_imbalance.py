...
class MOCAuctionImbalanceStrategy(AbstractStrategy):
    ...
    def backtest_signals(self, df: pd.DataFrame) -> BacktestSignals:
        ...
        # On a daily bar: sign = +1 if close > open (net buying), -1 if not
        bar_sign = np.sign(close - open_)
        signed_vol = bar_sign * volume

        # Normalise by 20-day rolling average volume
        ...
        # ── ATR volatility filter: only trade on adequately volatile days ─────
        tr = (high - low).combine(
