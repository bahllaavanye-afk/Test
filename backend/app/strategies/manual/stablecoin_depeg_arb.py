...
class StablecoinDepegArbStrategy(AbstractStrategy):
    ...
    def _prepare_price_series(self, df: pd.DataFrame) -> pd.Series | None:
        """Extract a clean price series or return None for invalid input."""
        if df is None or not isinstance(df, pd.DataFrame):
            return None
        if "close" not in df.columns or df.empty:
            return None
        # Ensure numeric conversion; drop rows that cannot be cast
    ...
    def backtest_signals(self, df: pd.DataFrame) -> BacktestSignals:
        # Defensive defaults for any edge‑case input
        ...
        price = self._prepare_price_series(df)
        if price is None or len(price) < self.lookback + self.min_persist:
            return default
        ...
        # Entry signals (shifted to avoid look‑ahead bias)
        ...
        # Exit signals – if we are already at the peg, we consider the position closed.
    ...
    async def analyze(self, df: pd.DataFrame, symbol: str) -> Signal | None:
        ...
        # Safely fetch the latest values; if NaN, abort.
        if prem_bps.empty or prem_z.empty:
            return None
        ...
        if current_prem < -self.entry_bps and current_z < -self.z_thresh:
            ...
            return Signal(
                ...
                take_profit=1.0,
                metadata={
                    "premium_bps": round(current_prem, 2),
