...
_unguarded_analyze = TLTSPYRotationStrategy.analyze
...
async def _failsoft_analyze(self, data, symbol: str = "SPY"):
    try:
        ...
        print(f"tlt_spy_rotation: analyze fail-soft -> None ({type(exc).__name__}: {str(exc)[:80]})")
        return None


TLTSPYRotationStrategy.analyze = _failsoft_analyze
