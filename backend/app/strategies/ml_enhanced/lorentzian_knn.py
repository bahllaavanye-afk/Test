...
class LorentzianStrategy(AbstractStrategy):
    ...
    This strategy builds a KNN model on Lorentzian‑transformed price features and
    generates trading signals based on model confidence, direction consistency, and
    a 20‑period simple moving average price filter.
    ...
    async def analyze(self, data: pd.DataFrame, symbol: str) -> Optional[Signal]:
        ...
        # Determine side based on probability and SMA alignment
        if prob > 0.5:
            if price <= sma20:
                logger.info(
                    "analyze_no_signal",
                    extra={
                        "symbol": symbol,
                        "reason": "price_below_sma_for_long",
                        "execution_time_ms": (time.perf_counter() - start_time) * 1000,
                    },
                )
                return None
            ...
            if price >= sma20:
                logger.info(
                    ...
                    extra={
                        "symbol": symbol,
                        "reason": "price_above_sma_for_short",
                        "execution_time_ms": (time.perf_counter() - start_time) * 1000,
                    },
                )
                return None
    ...
    def backtest_signals(self, df: pd.DataFrame) -> BacktestSignals:
        ...
        This method iterates over the DataFrame, applying the same logic as
        :meth:`analyze` but without the asynchronous constraints. It returns a
        :class:`BacktestSignals` container that holds timestamps, sides, and any
        ancillary information required for performance evaluation.

        Parameters
        ----------
        df : pd.DataFrame
            Historical OHLCV data for the back‑test period. Must contain a ``"close"``
            column.

        Returns
        -------
        BacktestSignals
            Collection of generated signals with associated metadata.
        """
        ...
        for i in range(1, len(df)):
            ...
            sma20 = df["close"].rolling(window=20).mean().iloc[i]
