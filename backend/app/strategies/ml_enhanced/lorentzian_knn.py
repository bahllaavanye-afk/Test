...
class LorentzianStrategy(AbstractStrategy):
    ...
    async def analyze(self, data: pd.DataFrame, symbol: str) -> Optional[Signal]:
        ...
        # Determine direction and apply price‑SMA alignment filter
        if prob > 0.5:
            if price <= sma20:
                logger.info(
                    "analyze_no_signal",
        ...
        signal = Signal(
            ...
            confidence=confidence,
            probability=prob,
            extra={"signal_id": self._signal_counter},
        )
        logger.info(
    ...
    def backtest_signals(self, data: pd.DataFrame) -> BacktestSignals:
        """
        Generate signals over a historical dataset for back‑testing purposes.

        The method re‑uses the same filtering logic as :meth:`analyze` but iterates over
        the entire DataFrame, returning a collection of signals together with auxiliary
        information required by the back‑testing engine.

        Parameters
        ----------
        data : pd.DataFrame
            Historical OHLCV data for the back‑test.

        Returns
        -------
        BacktestSignals
            Container holding all generated signals and any additional metadata.
        """
        start_time = time.perf_counter()
        ...
        for idx in range(1, len(data)):
            # Build feature vectors for current and previous rows
            latest_features = features[idx : idx + 1].astype(np.float32)
            prev_features = features[idx - 1 : idx].astype(np.float32) if idx > 0 else None
