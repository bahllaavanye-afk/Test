...
features generated by :func:`app.ml.models.lorentzian_knn.compute_lorentzian_features`.  
The strategy applies several filters (confidence, direction confirmation, and a 20‑period SMA
...
class LorentzianStrategy(AbstractStrategy):
    ...
    The strategy builds a KNN model on‑the‑fly from price data, evaluates the latest
    feature vector and, after applying confidence, direction‑confirmation and SMA‑20
    filters, emits a :class:`Signal` that can be used by the execution engine.
    ...
    async def analyze(self, data: pd.DataFrame, symbol: str) -> Optional[Signal]:
        ...
        bullish move, and applies a series of filters (confidence, direction confirmation,
        and SMA‑20 price filter). If all criteria are satisfied, a :class:`Signal` object
        ...
        # Determine signal direction based on model probability and SMA relationship
        if prob > 0.5:
            if price <= sma20:
                logger.info(
                    "analyze_no_signal",
    ...
    def backtest_signals(self, data: pd.DataFrame) -> BacktestSignals:
        """
        Generate signals for a full historical data series, suitable for back‑testing.

        This method mirrors the live ``analyze`` logic but iterates over the entire
        DataFrame, emitting a :class:`Signal` each time the filter criteria are met.

        Parameters
        ----------
        data : pd.DataFrame
            Historical OHLCV data for the back‑test period.
        ...
        """
        # Re‑use the existing model building logic; the model is created once.
        model = self._get_or_build_model(data)
        feat_df = compute_lorentzian_features(data)
        features = feat_df[LORENTZIAN_FEATURES].fillna(0).values
        ...
        for idx in range(1, len(features)):
            ...
            x_prev = torch.tensor(prev_features, dtype=torch.float32)
            prev_prob = float(model.forward(x_prev).item())
            if (prob > 0.5 and prev_prob <= 0.5) or (prob < 0.5 and prev_prob >= 0.5):
                continue

            # SMA‑20 filter
            window_slice = data.iloc[: idx + 1]
            sma20 = window_slice["close"].rolling(window=20).mean().iloc[-1]
            price = window_slice["close"].iloc[-1]
            if np.isnan(sma20):
                continue
