...
class AgentMemory:
    """Interface for storing and retrieving time‑stamped observations in Redis.

    The class abstracts a Redis client that supports asyncio commands. All
    observations are JSON‑encoded dictionaries that include a ``ts`` field with
    the Unix timestamp at which the observation was recorded.
    """
    ...
    def __init__(self, redis_client: Any) -> None:
        ...
        Args:
            redis_client: An async‑compatible Redis client (e.g. ``aioredis``) that
                provides ``lpush``, ``ltrim``, ``set``, ``get``, ``lrange``,
                and ``keys`` coroutine methods.
        """
    ...
    async def write(self, topic: str, data: Dict[str, Any]) -> None:
        ...
        The observation is JSON‑encoded and pushed onto the left side of the Redis
        list. The list is trimmed to ``_MAX_LIST_LEN`` items to prevent unbounded
        growth.

        Args:
            topic: The memory topic under which to store the observation.
            data: A dictionary of observation fields (excluding ``ts``).
    ...
    async def set_latest(self, topic: str, data: Dict[str, Any]) -> None:
        ...
        This stores a single JSON‑encoded observation under a ``latest:`` prefixed
        key, replacing any existing value.

        Args:
            topic: The memory topic to update.
            data: A dictionary of observation fields (excluding ``ts``).
        """
    ...
    async def read_recent(self, topic: str, n: int = 50) -> List[Dict[str, Any]]:
        ...
        Args:
            topic: The memory topic to query.
            n: Maximum number of observations to return.

        Returns:
            A list of dictionaries, each containing the stored fields and a ``ts``
            timestamp. Returns an empty list on error.
        """
        key = f"{_PREFIX}{topic}"
        try:
    ...
    async def get_latest(self, topic: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            The most recent observation as a dictionary, or ``None`` if the key
    ...
    async def read_all_topics(self) -> List[str]:
        ...
        Returns:
            A list of topic names (without the ``agent:memory:`` prefix). An empty
            list is returned if an error occurs.
        """
        try:
            pattern = f"{_PREFIX}*"
