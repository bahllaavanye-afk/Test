...
class AgentMemory:
    """Asynchronous interface for storing and retrieving observations in Redis."""
    ...
    async def get_latest(self, topic: str) -> Optional[Dict[str, Any]]:
        """Return the latest single‑value for a topic, or ``None`` if absent."""
        key = f"{_PREFIX}latest:{topic}"
        try:
    ...
    async def read_all_topics(self) -> List[str]:
        ...
        try:
            ...
            return [key.removeprefix(_PREFIX) for key in keys]
        except Exception as e:  # pragma: no cover
