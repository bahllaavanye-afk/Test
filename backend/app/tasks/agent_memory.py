...
MAX_LIST_LENGTH = 500  # cap per topic to avoid unbounded growth
LATEST_PREFIX = "latest:"
DEFAULT_READ_LIMIT = 50
ALL_TOPICS_PATTERN = f"{AGENT_MEMORY_PREFIX}*"
...
class AgentMemory:
    ...
    async def write(self, topic: str, data: dict) -> None:
        ...
        key = f"{AGENT_MEMORY_PREFIX}{topic}"
        try:
            await self._r.lpush(key, payload)
            await self._r.ltrim(key, 0, MAX_LIST_LENGTH - 1)
    ...
    async def set_latest(self, topic: str, data: dict) -> None:
        ...
        key = f"{AGENT_MEMORY_PREFIX}{LATEST_PREFIX}{topic}"
        payload = json.dumps({"ts": time.time(), **data})
        try:
    ...
    async def read_recent(self, topic: str, n: int = DEFAULT_READ_LIMIT) -> list[dict]:
    ...
    async def read_all_topics(self) -> list[str]:
        ...
        try:
            keys = await self._r.keys(ALL_TOPICS_PATTERN)
            return [k.removeprefix(AGENT_MEMORY_PREFIX) for k in keys]
        except Exception as e:
