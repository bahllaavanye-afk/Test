...
LONG_ENTRY_VALUE: int = 1
SHORT_ENTRY_VALUE: int = -1
...
async def run_backtest_job(run_id: str | None) -> None:
    ...
    try:
        ...
        if isinstance(raw_signals, _BSig):
            ...
            sig = pd.Series(DEFAULT_SIGNAL_VALUE, index=df.index, dtype=int)
            sig[entries.astype(bool)] = LONG_ENTRY_VALUE
            sig[exits.astype(bool)] = DEFAULT_SIGNAL_VALUE
