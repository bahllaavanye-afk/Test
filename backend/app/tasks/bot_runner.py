...
class BotRunner:
    ...
    def _has_job(self, bot_id: Optional[str]) -> bool:
    ...
    async def start(self, only_missing: bool = False) -> int:
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                bots: List[Bot] = result.scalars().all() or []

            # Guard against None or empty collections
            if not bots:
            ...
            pending = [
                b
                for b in bots
                if not (only_missing and self._has_job(b.id))
            ]
            logger.info(
    ...
    async def _run_bot(self, bot_id: Optional[str]) -> None:
    ...
    async def reschedule(self, bot: Optional["Bot"]) -> None:
        """Add or update a bot job in the scheduler."""
        if not bot:
            logger.debug("BotRunner.reschedule called with None bot")
            return
        try:
            ...
            if not bot.id:
                logger.error("BotRunner.reschedule: bot missing id", bot=bot)
                return
            ...
            logger.error(
                "BotRunner.reschedule failed", bot_id=getattr(bot, "id", None), error=str(exc)
            )

    async def unschedule(self, bot_id: Optional[str]) -> None:
