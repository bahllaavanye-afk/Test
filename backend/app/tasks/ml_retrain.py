...
_ALLOWED_INTERVALS = {
    "1m", "2m", "5m", "15m", "30m", "60m", "90m", "1h", "2h", "4h", "6h", "8h",
    "12h", "1d", "3d", "1wk", "1mo", "3mo"
}
...
def _validate_interval(interval: str) -> None:
    _validate_non_empty_str(interval, "interval")
    # Normalize to lower case for comparison
    normalized = interval.lower()
    if normalized not in _ALLOWED_INTERVALS:
        raise ValueError(
            f"interval '{interval}' is not supported. Allowed intervals are: "
            f"{', '.join(sorted(_ALLOWED_INTERVALS))}."
        )
...
async def retrain_model(model_name: str, symbol: str, interval: str = "1h") -> dict:
    ...
    # Input validation
    _validate_non_empty_str(model_name, "model_name")
