"""
Nightly ML retraining: downloads fresh data, retrains all active models,
compares new vs old Sharpe, promotes if improved.
"""
from __future__ import annotations

import asyncio
import re
from datetime import datetime, timezone, timedelta
from pathlib import Path
from typing import Any, List, Tuple

import pandas as pd

from app.utils.logging import logger

ARTIFACTS_DIR = Path(__file__).parents[3] / "models_artifacts"


def _is_invalid_str(value: Any) -> bool:
    """Return True if a string argument is None, empty, or only whitespace."""
    return not isinstance(value, str) or not value.strip()


async def retrain_model(model_name: str, symbol: str, interval: str = "1h") -> dict:
    """Download 2 years of data and retrain a model. Returns result dict.

    Handles None/empty inputs gracefully and reports a skipped status.
    """
    # Validate inputs early
    if _is_invalid_str(model_name):
        logger.warning("Retrain skipped due to invalid model name", model=model_name)
        return {"status": "skipped", "reason": "invalid model name"}
    if _is_invalid_str(symbol):
        logger.warning("Retrain skipped due to invalid symbol", symbol=symbol)
        return {"status": "skipped", "reason": "invalid symbol"}
    if _is_invalid_str(interval):
        logger.warning("Retrain skipped due to invalid interval", interval=interval)
        return {"status": "skipped", "reason": "invalid interval"}

    try:
        import yfinance as yf

        loop = asyncio.get_running_loop()
        end = datetime.now(timezone.utc)
        start = end - timedelta(days=730)

        hist = await loop.run_in_executor(
            None,
            lambda: yf.download(
                symbol,
                start=str(start.date()),
                end=str(end.date()),
                interval=interval,
                auto_adjust=True,
                progress=False,
            ),
        )
        # Ensure we have enough rows and a non‑empty DataFrame
        if hist is None or len(hist) < 200 or hist.empty:
            return {"status": "skipped", "reason": "insufficient data"}

        # Normalize column names safely
        if isinstance(hist.columns, pd.MultiIndex):
            # Flatten MultiIndex by joining levels with '_' and lower‑casing
            hist.columns = [
                "_".join(map(str, col)).lower() for col in hist.columns
            ]
        else:
            hist.columns = [
                c.lower() if isinstance(c, str) else str(c).lower()
                for c in hist.columns
            ]

        from app.ml.training.train_lstm import train

        experiment_name = f"{model_name}_{symbol.lower()}_{datetime.now(timezone.utc).strftime('%Y%m%d')}"
        result = await train(hist, experiment_name=experiment_name, max_epochs=30)
        result["symbol"] = symbol
        result["model"] = model_name
        result["retrained_at"] = datetime.now(timezone.utc).isoformat()
        logger.info(
            "Model retrained",
            **{k: v for k, v in result.items() if k != "best_model_path"},
        )
        return result

    except Exception as e:
        logger.error(
            "Retrain failed",
            model=model_name,
            symbol=symbol,
            error=str(e),
        )
        return {"status": "error", "error": str(e)}


def _load_retrain_configs() -> List[Tuple[str, str, str]]:
    """
    Discover retrain targets dynamically from experiment configs (*.yaml).
    Falls back to a minimal default set if no configs exist or yaml is unavailable.
    Returns list of (model_name, symbol, interval).
    """
    configs_dir = Path(__file__).parents[3] / "experiments" / "configs"
    seen: set[Tuple[str, str, str]] = set()
    results: List[Tuple[str, str, str]] = []

    if not configs_dir.is_dir():
        # No config directory – use defaults
        return [
            ("lstm", "BTC-USD", "1h"),
            ("lstm", "ETH-USD", "1h"),
            ("lstm", "SPY", "1d"),
        ]

    try:
        import yaml as _yaml

        _load_yaml = _yaml.safe_load
    except ImportError:
        _load_yaml = None

    for cfg_path in sorted(configs_dir.glob("*.yaml")):
        try:
            with open(cfg_path, "r", encoding="utf-8") as f:
                if _load_yaml:
                    cfg = _load_yaml(f)
                else:
                    # Minimal fallback: regex‑extract model/symbol/interval from YAML text
                    text = f.read()
                    cfg = {
                        "experiment": {
                            k: v
                            for k, v in re.findall(
                                r"^\s{2}(model|symbol|interval):\s*['\"]?([^\s'\"#]+)",
                                text,
                                re.MULTILINE,
                            )
                        }
                    }

            exp = (cfg or {}).get("experiment", {})
            model = exp.get("model", "lstm")
            symbol = exp.get("symbol", "SPY")
            interval = exp.get("interval", "1d")

            # Guard against malformed entries
            if _is_invalid_str(model) or _is_invalid_str(symbol) or _is_invalid_str(interval):
                continue

            key = (model, symbol, interval)
            if key not in seen:
                seen.add(key)
                results.append(key)
        except Exception:
            # Skip problematic config files silently; they will be logged elsewhere if needed
            continue

    if not results:
        results = [("lstm", "BTC-USD", "1h"), ("lstm", "ETH-USD", "1h"), ("lstm", "SPY", "1d")]

    return results


async def nightly_retrain() -> None:
    """Retrain all models discovered from experiment configs. Called by APScheduler at 02:00 UTC."""
    retrain_configs = _load_retrain_configs()
    # Cap at 10 per night to avoid overwhelming free‑tier CPU
    retrain_configs = retrain_configs[:10] if retrain_configs else []

    logger.info("Nightly retrain starting", configs=len(retrain_configs))

    results = await asyncio.gather(
        *[retrain_model(m, s, i) for m, s, i in retrain_configs],
        return_exceptions=True,
    )

    successes = 0
    for r in results:
        if isinstance(r, dict):
            # Consider a run successful only if it completed without error and wasn't skipped
            if r.get("status") not in {"error", "skipped"}:
                successes += 1
        # Non‑dict results are exceptions; they are counted as failures

    logger.info(
        "Nightly retrain complete",
        total=len(retrain_configs),
        succeeded=successes,
    )