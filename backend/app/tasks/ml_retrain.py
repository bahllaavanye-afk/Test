...
async def retrain_model(model_name: str | None, symbol: str | None, interval: str = "1h") -> dict:
    ...
    Handles None or empty inputs gracefully, returning a skipped status.
    """
    # Validate inputs
    if not model_name or not symbol:
        return {"status": "skipped", "reason": "invalid model_name or symbol"}
    ...
    try:
        ...
        # Guard against None or insufficient rows
...
def _load_retrain_configs() -> list[tuple[str, str, str]]:
    ...
    for cfg_path in sorted(configs_dir.glob("*.yaml")):
        try:
            ...
            if not model or not symbol or not interval:
                continue
            ...
            # Skip malformed config files without interrupting discovery
            continue
...
async def nightly_retrain() -> None:
    ...
    # Guard against None (should not happen) and cap at 10 per night
    if not retrain_configs:
        logger.info("Nightly retrain skipped: no configs found")
        return
