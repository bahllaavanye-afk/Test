...
def parse_test_failures(pytest_output: str | None) -> list[FailureRecord]:
    ...
    for line in lines:
        ...
        if line_number is not None and line_number <= 0:
            line_number = None  # Guard against invalid line numbers
...
def scan_security_issues() -> list[SecurityIssue]:
    ...
    patterns = [
        ...
        (r'execute\s*\(\s*f["\']', "high", "sql_injection_risk",
         "f-string in exec/cursor call may lead to SQL injection", False),
    ]

    if not patterns:
        return issues

    # Walk through all Python files under the project root
    for py_path in PROJECT_ROOT.rglob("*.py"):
        try:
            content = py_path.read_text(encoding="utf-8", errors="ignore")
        except Exception as e:
            logger.warning(f"Failed to read {py_path}: {e}")
            continue

        for pat, severity, issue_type, description, auto_fixable in patterns:
            for match in re.finditer(pat, content):
                # Compute line number (1‑based). Guard against off‑by‑one errors.
...
# Note: The rest of the QA monitor orchestration (auto‑fix, reporting, scheduling) is
# defined elsewhere in the codebase. This module now robustly handles None inputs,
# empty collections, and guards against off‑by‑one line number calculations.