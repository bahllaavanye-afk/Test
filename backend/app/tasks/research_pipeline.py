...
class ResearchPipeline:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            logger.exception("ResearchPipeline unexpected error during run")
    ...
    async def _build_market_context(self) -> str:
        ...
        recent_suggestions = await self._memory.read_recent(
            "llm_suggestions", n=3
        )

        regime = regime_data.get("regime", "unknown") if regime_data else "unknown"
        health = (
            platform_data.get("health_ratio", 0.5) if platform_data else 0.5
        )
        prev_ideas = [
            s.get("suggestion", "")[:100] for s in recent_suggestions
        ]
...
[{{"name": "idea_name", "hypothesis": "...", "model": "lstm|xgboost|manual", "symbol": "BTC/USDT|SPY", "interval": "1h|1d", "features": ["rsi_14", ...]}}]"""
        ...
        if not response:
            logger.warning("ResearchPipeline: LLM returned no response")
            return []
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                logger.error(
                    "ResearchPipeline: LLM response does not contain JSON array",
                    extra={"response": content},
                )
        except json.JSONDecodeError as jde:
            logger.exception(
                "ResearchPipeline: JSON decode error while parsing LLM ideas",
                extra={"error": str(jde)},
            )
        except Exception as e:
            logger.exception(
                "ResearchPipeline: unexpected error while parsing LLM ideas",
                extra={"error": str(e)},
            )
        return []
    ...
    async def _ideas_to_experiment_configs(self, ideas: List[dict]) -> List[Path]:
        ...
        for idea in ideas[:2]:  # limit to 2 per cycle
            ...
            if config_path.exists():
                logger.info(
                    "ResearchPipeline: config already exists, skipping",
                    extra={"config": config_path.name},
                )
                continue
...
"""
            try:
                ...
                logger.info(
                    "ResearchPipeline: created config %s",
                    config_path.name,
                    extra={"config": config_path.name},
                )
            except OSError as ose:
                logger.exception(
                    "ResearchPipeline: failed to write config file",
                    extra={"config": config_path.name, "error": str(ose)},
                )
        return configs
    ...
    async def _queue_experiments(self, configs: List[Path]) -> None:
        ...
        if not script.exists():
            logger.error(
                "ResearchPipeline: run_experiment.py not found",
                extra={"script_path": str(script)},
            )
            return
        ...
        for config in configs:
            try:
                ...
                logger.info(
                    "ResearchPipeline: queued experiment %s",
                    config.name,
                    extra={"config": config.name},
                )
            except OSError as ose:
                logger.exception(
                    "ResearchPipeline: OS error while queuing experiment",
                    extra={"config": config.name, "error": str(ose)},
                )
            except Exception as e:
                logger.exception(
                    "ResearchPipeline: unexpected error while queuing experiment",
                    extra={"config": config.name, "error": str(e)},
                )