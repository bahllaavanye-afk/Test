...
def start_scheduler(
    ...
    Args:
        db_session_factory: Callable that returns an async SQLAlchemy session.
            It may be ``None`` in which case the function will attempt to import
            the default session factory from ``app.database``.
    ...
    Raises:
        ValueError: If ``db_session_factory`` is provided but not callable.
    """
    # Input validation
    ...
    if broker is not None:
        # No strict contract for broker yet; ensure it's not a primitive type.
        if isinstance(broker, (str, int, float, bool)):
            raise ValueError("broker, if provided, must be an object, not a primitive type.")
    ...
    async def _order_sync() -> None:
        ...
        # The detailed implementation of order synchronization depends on the
        # broker-specific APIs and database models. To keep this module robust
        # and avoid runtime failures when the implementation is incomplete, we
        # perform a safe‑guarded import and log a placeholder message if the
        # necessary components are unavailable.
        try:
            # Example placeholders – replace with real imports when the feature is ready.
            from app.models.order import Order  # type: ignore
            from app.brokers.alpaca_orders import sync_alpaca_orders  # type: ignore

            async with factory() as db:
                await sync_alpaca_orders(db)
            logger.info("Order sync completed")
        except Exception as exc:
            logger.error("Order sync failed", error=str(exc))

    # Register jobs with validation via SchedulerJobConfig
    job_definitions = [
        SchedulerJobConfig(
        ...
        SchedulerJobConfig(
            ...
            func=_order_sync,
            description="Synchronize open broker orders every minute.",
        ),
    ]

    for cfg in job_definitions:
        _add_job(scheduler, cfg)
