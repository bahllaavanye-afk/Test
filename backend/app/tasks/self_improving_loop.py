...
class SelfImprovingLoop:
    ...
    async def _auto_disable_underperformers(self, metrics: List[Dict] | None) -> None:
        ...
        underperformers = [
            m
            for m in metrics
            if (m.get("sharpe") or 0) < 0 and (m.get("num_trades") or 0) >= 10
        ]
    ...
    async def _llm_improvement_pass(self, metrics: List[Dict] | None) -> None:
        ...
        # Safely sort even if some Sharpe values are missing or None
        sorted_by_sharpe = sorted(
            metrics,
            key=lambda m: m.get("sharpe", 0) if m.get("sharpe") is not None else 0,
            reverse=True,
        )
        top = sorted_by_sharpe[:5]
...
Be concise. Each suggestion under 2 sentences."""
    ...
    async def _broadcast_regime(self, metrics: List[Dict] | None) -> None:
        if not metrics:
            # No data – default to neutral regime
            regime = "sideways"
            health = 0.0
            profitable = 0
            total = 0
        else:
            profitable = sum(1 for m in metrics if (m.get("sharpe") or 0) > 0.5)
            total = len(metrics)
