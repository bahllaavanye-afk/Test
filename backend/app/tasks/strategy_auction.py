...
_MIN_PULLS_FOR_EXPLOIT = 5
_PROBATION_THRESHOLD_DAYS = 7
_PROBATION_ALLOCATION = 0.01

_NEW_STRATEGY_BONUS = 10.0
_MIN_SCORE = 0.001

_TOP_TIER_PERCENT = 30            # percentage of non‑probation strategies in top tier
_TOP_TIER_SHARE = 0.70            # fraction of available capital for top tier
_REST_TIER_SHARE = 0.30           # fraction of available capital for rest tier

_STATE_TTL_SECONDS = 86400 * 30   # 30 days
_ALLOCATION_TTL_SECONDS = 7200    # 2 hours
...
class StrategyBid:
    ...
    def ucb1_score(self, total_pulls: int) -> float:
        if self.pulls < _MIN_PULLS_FOR_EXPLOIT:
            ...
            return _NEW_STRATEGY_BONUS + (1.0 / (self.pulls + 1))
...
class StrategyAuction:
    ...
    async def _save_state(self) -> None:
        try:
            ...
            await self._r.set(_AUCTION_STATE_KEY, json.dumps(data), ex=_STATE_TTL_SECONDS)
        except Exception as e:
    ...
    async def run_auction(self) -> dict[str, float]:
        ...
        # Separate probation and non‑probation strategies
        ...
        # Determine top tier count (at least one strategy)
        top_count = max(1, len(non_probation) * _TOP_TIER_PERCENT // 100)
        top_strategies = non_probation[:top_count]
        rest_strategies = non_probation[top_count:]

        top_capital = available_capital * _TOP_TIER_SHARE
        rest_capital = available_capital * _REST_TIER_SHARE
        ...
        def distribute(strategies: list[tuple[str, float]], budget: float) -> dict[str, float]:
            if not strategies:
                return {}
            total_score = sum(max(s, _MIN_SCORE) for _, s in strategies)
            return {n: budget * max(s, _MIN_SCORE) / total_score for n, s in strategies}
        ...
        try:
            await self._r.set(_ALLOCATION_KEY, json.dumps(allocations), ex=_ALLOCATION_TTL_SECONDS)
        except Exception as e:
