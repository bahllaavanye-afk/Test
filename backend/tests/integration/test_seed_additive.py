...
async def test_seed_is_additive_and_idempotent(client, monkeypatch):
    ...
    # A full seed is asserted again (rather than tolerating "already seeded"),
    # because conftest._isolate_each_file now gives every test FILE a clean
    # database. The tolerant version was a workaround for the missing
    # isolation; with real isolation the strict assert is both correct and
    # stronger — it verifies seeding actually does the work.
