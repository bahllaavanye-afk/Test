...
Edge‑case tests ensure graceful handling of None, empty inputs and off‑by‑one scenarios.
"""
import math
import pytest
import numpy as np
...
def _make_entry_exit(prices: pd.Series, buy_every: int = 10) -> tuple[pd.Series, pd.Series]:
    """Create boolean entry/exit series. Handles very short price series safely."""
    entries = pd.Series(False, index=prices.index)
    exits = pd.Series(False, index=prices.index)
    # Guard against buy_every larger than the series length
    if len(prices) <= buy_every:
        return entries, exits
    for i in range(0, len(prices) - buy_every, buy_every * 2):
        entries.iloc[i] = True
        # Ensure the exit index exists
        exit_idx = i + buy_every
...
def test_compute_metrics_none_returns_no_crash():
    m = compute_metrics(None)
    # Expect a zeroed dataclass; sharpe is a safe sentinel
    assert isinstance(m, PerformanceMetrics)
    assert m.sharpe == 0.0
...
def test_entries_exits_none_do_not_crash():
    r = _daily_returns()
    m = compute_metrics(r, entries=None, exits=None)
    assert isinstance(m, PerformanceMetrics)


def test_entries_exits_empty_series_do_not_crash():
    r = _daily_returns()
    empty = pd.Series([], dtype=bool)
    m = compute_metrics(r, entries=empty, exits=empty)
    assert isinstance(m, PerformanceMetrics)


def test_entries_exits_length_mismatch():
    r = _daily_returns()
    prices = _prices_from_returns(r)
    entries, exits = _make_entry_exit(prices)
    # Truncate entries to create a length mismatch
    short_entries = entries.iloc[:-5]
    m = compute_metrics(r, entries=short_entries, exits=exits)
    assert isinstance(m, PerformanceMetrics)


def test_make_entry_exit_small_series():
    # Series shorter than buy_every – should not raise IndexError
    prices = pd.Series([100, 101], index=pd.date_range("2022-01-03", periods=2, freq="B"))
    entries, exits = _make_entry_exit(prices, buy_every=5)
...
def test_compute_metrics_none_benchmark_and_signal():
    r = _daily_returns()
    m = compute_metrics(r, benchmark=None, signal=None)
    assert isinstance(m, PerformanceMetrics)
...
def test_ic_bounds_when_signal_is_none():
    r = _daily_returns(drift=0.001)
    m = compute_metrics(r, signal=None)
    # When signal is None, IC should be a safe default (e.g., 0)
    assert m.ic == 0.0