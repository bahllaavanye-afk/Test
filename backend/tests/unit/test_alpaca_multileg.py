...
@pytest.fixture
def account():
    """Simple account namespace used across tests."""
...
@pytest.fixture
def mock_headers():
    """Patch _headers to return an empty dict (no auth needed for unit tests)."""
    with patch("app.brokers.alpaca_orders._headers", new=AsyncMock(return_value={})):
        yield


@pytest.fixture
def mock_base_url():
    """Patch base URL to the paper endpoint."""
    with patch("app.brokers.alpaca_orders._base_url", return_value="https://paper-api.alpaca.markets"):
        yield
...
def test_pick_contract_variants(snaps, target, kind, expected):
    assert pick_contract_by_delta(snaps, target, kind) == expected
...
@pytest.fixture
def capture_payload():
    """Capture the URL and JSON payload sent to httpx.AsyncClient.post."""
    captured = {}
...
@pytest.fixture
def capture_post_no_send():
    """Capture any URLs posted when a leg cannot be resolved (should be none)."""
    posted = []
...
def test_multileg_builds_mleg_payload(account, mock_headers, mock_base_url, capture_payload):
    legs = [
        {"side": "sell", "option_type": "put", "dte": 30, "delta": 0.16},
        {"side": "buy", "option_type": "put", "dte": 30, "delta": 0.10, "ratio": 1},
    ]
    resolve_mock = AsyncMock(side_effect=["SPY260814P00580000", "SPY260814P00560000"])
    with patch("app.brokers.alpaca_orders.resolve_leg_symbol", new=resolve_mock):
        out = pytest.run(asyncio.run(submit_alpaca_multileg_order(account, "SPY", legs, quantity=2)))

    assert out and out["id"] == "mleg-1"
    p = capture_payload["payload"]
    assert p["order_class"] == "mleg"
    assert p["qty"] == "2"
    assert capture_payload["url"].endswith("/v2/orders")
...
def test_multileg_unresolvable_leg_sends_nothing(account, capture_post_no_send):
    legs = [
        {"side": "sell", "option_type": "put", "dte": 30, "delta": 0.16},
        {"side": "buy", "option_type": "put", "dte": 30, "delta": 0.10},
    ]
    resolve_mock = AsyncMock(side_effect=["SPY260814P00580000", None])
    with patch("app.brokers.alpaca_orders.resolve_leg_symbol", new=resolve_mock):
        out = pytest.run(asyncio.run(submit_alpaca_multileg_order(account, "SPY", legs)))

    assert out is None
    assert capture_post_no_send == []          # NEVER send a partial spread


def test_multileg_broker_rejection_returns_none(account, mock_headers, mock_base_url):
    async def fake_post(self, url, json=None, headers=None):
        return _Resp(422, {"message": "option level too low"})

    legs = [{"side": "buy", "option_type": "call", "dte": 30, "delta": 0.5}]
    resolve_mock = AsyncMock(return_value="SPY260814C00620000")
    with patch("app.brokers.alpaca_orders.resolve_leg_symbol", new=resolve_mock), \
         patch("httpx.AsyncClient.post", new=fake_post):
        result = pytest.run(asyncio.run(submit_alpaca_multileg_order(account, "SPY", legs)))
