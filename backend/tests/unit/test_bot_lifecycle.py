...
def test_profitable_enabled_bot_untouched():
    a = decide_bot_actions(
        [_s("winner", trades=20, pnl=800.0, wr=0.7)], []
    )
    assert a["disable"] == [] and a["enable"] == []


def test_profitable_bot_with_low_win_rate_is_disabled():
    # Even with profit, a win rate below the disable threshold signals poor quality.
    a = decide_bot_actions(
        [
            _s(
                "risky",
                trades=MIN_TRADES_TO_JUDGE,
                pnl=500.0,
                wr=DISABLE_WIN_RATE - 0.01,
            )
        ],
        [],
    )
    assert [b.name for b in a["disable"]] == ["risky"]


def test_bot_with_win_rate_just_above_threshold_survives():
    # Win rate marginally above the disable threshold should keep the bot alive.
    a = decide_bot_actions(
        [
            _s(
                "borderline",
...
def test_bot_with_insufficient_trades_and_low_metrics_not_disabled():
    # Bots lacking enough trade history should not be churned, even if metrics look bad.
    a = decide_bot_actions(
        [_s("newbie", trades=MIN_TRADES_TO_JUDGE - 1, pnl=-200.0, wr=0.2)], []
    )
    assert a["disable"] == [] and a["enable"] == []