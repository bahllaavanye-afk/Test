...
def _make_returns(corr_matrix: np.ndarray, n: int = 100, seed: int = 42) -> pd.DataFrame:
    """Generate synthetic returns with a given correlation structure.

    Args:
        corr_matrix: Target correlation matrix (square, positive‑definite).
        n: Number of observations to generate.
        seed: Random seed for reproducibility.

    Returns:
        DataFrame of shape (n, n_assets) with synthetic returns.
    """
