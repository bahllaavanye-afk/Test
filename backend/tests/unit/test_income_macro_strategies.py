...
# ----------------------------------------------------------------------
# Caches to avoid repeated expensive strategy instantiation and backtest
# ----------------------------------------------------------------------
_INSTANCE_CACHE: dict[str, object] = {}
_SIGNALS_CACHE: dict[tuple[str, int], BacktestSignals] = {}

def _get(name: str):
    """Retrieve strategy instance, caching to avoid repeated construction."""
...
def _cached_signals(name: str, df: pd.DataFrame) -> BacktestSignals:
    """Cache backtest signals per strategy and dataframe shape."""
    key = (name, df.shape[0])
    if key not in _SIGNALS_CACHE:
        inst = _get(name)
        _SIGNALS_CACHE[key] = inst.backtest_signals(df)
...
def test_backtest_signals_shape(name, daily_ohlcv):
    sig = _cached_signals(name, daily_ohlcv)
