...
function exportToCSV(trades: any[]) {
  ...
  const rows = trades.map(t => [
    ...
    t.entry_price ?? t.avg_fill_price ?? '', t.exit_price ?? '',
    t.realized_pnl ?? '', t.pnl_pct ?? '', t.hold_time ?? '',
...
export default function PnL() {
  ...
  function SortHeader({ label, col }: { label: string; col: SortKey }) {
    ...
    return (
      <th className="text-left px-4 py-3 cursor-pointer select-none group" onClick={() => handleSort(col)}>
        <span className="flex items-center gap-1">
          <span className={active ? 'text-[#f5a623]' : 'text-[#555] group-hover:text-[#888]'} style={{ transition: 'color 0.15s' }}>{label}</span>
          <span className={active ? 'text-[#f5a623]' : 'text-[#333]'}>{active ? (sortDir === 'asc' ? '↑' : '↓') : '↕'}</span>
        </span>
      </th>
    )
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 active:scale-95"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {perfLoading ? (
          ...
          <>
            <KpiCard label="Total P&L" value={`${totalPnl >= 0 ? '+' : ''}$${Math.abs(totalPnl).toFixed(2)}`}
              color={totalPnl >= 0 ? '#00c853' : '#ff1744'} sub={`${tradeList.length} total trades`} />
            <KpiCard label="Win Rate" value={`${winRate}%`}
              color={parseFloat(winRate) >= 50 ? '#00c853' : '#ff1744'} sub={`${winCount}W / ${lossCount}L`} />
            <KpiCard label="Avg P&L / Trade" value={`${avgPnl >= 0 ? '+' : ''}$${Math.abs(avgPnl).toFixed(2)}`}
              color={avgPnl >= 0 ? '#00c853' : '#ff1744'} />
            <KpiCard label="Best Trade" value={bestTrade === -Infinity ? '—' : `+$${bestTrade.toFixed(2)}`}
              color="#00c853" sub={worstTrade === Infinity ? '' : `Worst: -$${Math.abs(worstTrade).toFixed(2)}`} />
          </>
        )}
      </div>

      {/* Slippage by Algo — only show if real data */}
      {Array.isArray(slippage) && slippage.length > 0 && (
      ...
      <div className="bg-[#111111] border border-[#1e1e1e] rounded-lg overflow-hidden">
        <div className="p-3 border-b border-[#1e1e1e] flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-[#0a0a0a] border border-[#1e1e1e] rounded-lg p-1">
            {(['all', 'buy', 'sell'] as const).map(f => (
              <button key={f} onClick={() => { setFilterSide(f); setPage(0) }}
                className="text-xs px-2 py-0.5 rounded capitalize transition-colors"
                style={{ background: filterSide === f ? (f === 'buy' ? '#00c853' : f === 'sell' ? '#ff1744' : '#f5a623') : 'transparent', color: filterSide === f ? (f === 'all' ? '#000' : '#fff') : '#555' }}>
                {f}
              </button>
            ))}
          </div>
        ...
        {tradesLoading ? (
          <div className="p-8 space-y-2">
            {Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-8 bg-[#1e1e1e] rounded animate-pulse" />)}
          </div>
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-[#888] text-sm font-medium">
              {tradeList.length === 0
                ? 'No trades yet. Strategies will execute their first trade when market data is connected.'
                : 'No trades match your filter'}
            </p>
            <p className="text-[#555] text-xs mt-1">
              {tradeList.length === 0
                ? 'Connect your Alpaca account in Settings to start paper trading.'
              ...
              <table className="w-full min-w-[800px]">
                ...
                <tbody>
                  {pagedTrades.map((t: any, i: number) => (
                    <tr key={t.id ?? i} className="border-t border-[#1e1e1e] hover:bg-[#0d0d0d] transition-colors">
                      ...
                      <td className="px-4 py-2.5"><SideChip side={t.side ?? ''} /></td>
                      ...
                      <td className="px-4 py-2.5 text-xs text-right"><PnlCell value={t.realized_pnl} /></td>
                      <td className="px-4 py-2.5 text-xs text-right"><PctCell value={t.pnl_pct} /></td>
                      <td className="px-4 py-2.5 text-xs text-right font-mono text-[#888]">
              ...
              <div className="border-t border-[#1e1e1e] px-4 py-2.5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                    className="px-2.5 py-1 text-xs rounded bg-[#1e1e1e] text-[#888] disabled:opacity-40 hover:bg-[#2e2e2e] transition-colors">←</button>
                  {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
                    ...
                    return (
                      <button key={p} onClick={() => setPage(p)} className="w-7 h-7 text-xs rounded transition-colors"
                        style={{ background: page === p ? '#f5a623' : '#1e1e1e', color: page === p ? '#000' : '#888' }}>
                        {p + 1}
                      </button>
                    )
                  })}
